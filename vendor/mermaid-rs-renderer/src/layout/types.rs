...
#[derive(Debug, Clone)]
pub struct ErTableLayout {
    pub title: TextBlock,
    pub rows: Vec<ErRowLayout>,
    pub columns: [f32; 4],
    pub padding: f32,
...
#[derive(Debug, Clone)]
pub struct ErRowLayout {
    pub name: TextBlock,
    pub data_type: TextBlock,
    pub comment: TextBlock,
    pub keys: Vec<(String, f32)>,
    pub top: f32,
    pub height: f32,
}
...
pub struct NodeLayout {
    pub er_table: Option<ErTableLayout>,
    pub id: String,
    pub x: f32,
    pub y: f32,
    pub width: f32,
...
pub struct SequenceNumberLayout {
    pub x: f32,
    pub y: f32,
    pub value: crate::ir::SequenceNumber,
    pub width: f32,
    pub height: f32,
}
...
#[derive(Debug, Clone)]
pub struct Layout {
    pub frontmatter_title: Option<DiagramTitle>,
...
pub struct C4RelLayout {
    ...
    /// The optional quadratic control point is part of the layout contract.
    pub control: Option<(f32, f32)>,
    pub label_center: (f32, f32),
    pub techn_center: Option<(f32, f32)>,
    pub label_bounds: C4Rect,
...
pub struct GanttLayout {
    ...
    pub excluded_spans: Vec<(f32, f32)>,
    pub today_marker: Option<(f32, crate::ir::GanttTodayMarker)>,
    pub compact: bool,
}
...
impl SubgraphLayout {
    /// Text bounds shared by SVG placement and routing obstacles.
    pub(crate) fn label_bounds(
        &self,
        kind: crate::ir::DiagramKind,
        theme: &crate::theme::Theme,
    ) -> (f32, f32, f32, f32) {
        let width = self.label_block.width;
        let height = self.label_block.height;
        let (x, y) = if kind == crate::ir::DiagramKind::State {
            let header_height = (height + theme.font_size * 0.75).max(theme.font_size * 1.4);
            (
                self.x + (theme.font_size * 0.6).max(height * 0.35),
                self.y + (header_height - height) * 0.5,
            )
        } else {
            (self.x + (self.width - width) * 0.5, self.y + 12.0)
        };
        (x, y, width, height)
    }
}
