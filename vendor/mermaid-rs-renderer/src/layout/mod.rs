...
use sequence::*;
use subgraphs::*;
pub(crate) use text::split_lines;
use text::*;
use timeline::*;
use treemap::*;
pub use types::*;
...
pub fn compute_layout_with_metrics(
    ...
    layout.frontmatter_title = graph.frontmatter_title.as_ref().map(|title| {
        let font_size = theme.font_size * 1.2;
        DiagramTitle {
            text: text::measure_label_with_font_size(
                title,
                font_size,
                config,
                false,
...
fn compute_flowchart_layout(
    ...
    for node in graph.nodes.values() {
        ...
        let mut placed = build_node_layout(node, label, width, height, style, graph);
        if graph.kind == crate::ir::DiagramKind::Er {
            let table = er::measure_table(&node.label, theme, config);
            placed.width = table.width;
            placed.height = table.height;
            placed.er_table = Some(table);
        }
        nodes.insert(node.id.clone(), placed);
    }
    ...
    if matches!(
        ...
        // Cardinality symbols, diamonds and inheritance heads occupy the gap.
        // Reserve their 20pt extent plus clearance around the relationship label.
        let label_extent = graph
            .edges
            .iter()
            .filter_map(|edge| edge.label.as_deref())
            .map(|label| {
                let label = measure_label(label, theme, &effective_config);
            ...
            .fold(0.0f32, f32::max);
        effective_config.rank_spacing = effective_config.rank_spacing.max(label_extent + 64.0);
    }
