...
pub(super) fn compute_sequence_layout(
    ...
    for idx in 0..=graph.edges.len() {
        if idx > 0 && graph.edges[idx - 1].from == graph.edges[idx - 1].to {
            // A self message finishes below its send row. Subsequent content
            // starts after that return segment, including notes and frames.
    ...
    for (id, index) in &graph.sequence_created {
        if let (Some(node), Some(y)) = (nodes.get_mut(id), message_ys.get(*index)) {
            node.y = y - actor_height / 2.0;
        }
    }
    ...
    if !graph.sequence_frames.is_empty() && !message_ys.is_empty() {
        ...
        for frame in frames {
            ...
            for (id, index) in &graph.sequence_created {
                if *index >= frame.start_idx && *index < frame.end_idx {
    ...
    for edge in &edges {
        for point in &edge.points {
            last_message_y = last_message_y.max(point.1);
        }
    }
    for note in &sequence_notes {
    ...
    let sequence_footboxes = participants
        .iter()
        .filter(|id| !graph.sequence_destroyed.contains_key(*id))
        .filter_map(|id| nodes.get(id))
        .map(|node| {
    ...
    for (idx, edge) in graph.edges.iter().enumerate() {
        if let Some(value) = graph.sequence_message_numbers.get(idx).copied().flatten() {
