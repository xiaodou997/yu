...
pub(super) fn compute_gitgraph_layout(
    ...
    for (index, (branch, _order)) in branch_entries.iter().enumerate() {
        ...
        let (bg_final_x, bg_final_y, text_x, text_y) = if is_vertical {
            ...
            // The old expression subtracted half the label height twice,
            // placing lower lines outside the background for multiline names.
            let bg_final_y = pos - label_height / 2.0 + gg.branch_label_bg_offset_y;
