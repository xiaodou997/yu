...
pub fn resolve_all_label_positions(
    ...
    //
    // Graph center labels may need to stack outside the initial routing
    // rectangle, especially beside endpoint symbols or parallel edges. A graph
    // bounds finalization pass runs after label placement and grows/translates
    // the layout to include labels, so do not clamp graph center-label
    ...
    let center_bounds = if matches!(layout.diagram, super::DiagramData::Graph { .. }) {
        None
    } else {
        bounds
    };
...
fn resolve_center_labels(
    ...
    for (idx, edge) in edges.iter_mut().enumerate() {
        ...
        if kind == DiagramKind::Flowchart
            || occupied
                .iter()
                .any(|other| overlap_area(&occupied_rect, other) > LABEL_OVERLAP_WIDE_THRESHOLD)
        {
            // Routing seeds cannot fix labels over nodes or endpoint symbols.
...
fn tighten_center_label_gaps(
    ...
    for _ in 0..iterations {
        ...
        for (idx, _) in order {
            ...
            let current_static_overlap: f32 = static_obstacles
                .iter()
                .map(|obstacle| overlap_area(&current_rect, obstacle))
                .sum();
            ...
            let evaluate = |center: (f32, f32),
                ...
                let static_overlap: f32 = static_obstacles
                    .iter()
                    .map(|obstacle| overlap_area(&rect, obstacle))
                    .sum();
                if static_overlap > current_static_overlap + LABEL_OVERLAP_WIDE_THRESHOLD {
                    return;
                }
...
fn enforce_center_label_attachment_caps(
    ...
    for _ in 0..2 {
        ...
        for (idx, edge) in edges.iter_mut().enumerate() {
            ...
            // When the closest segment is crowded, attachment may need to
            // move along the route as well as toward it. Sampling only the
            // nearest normal forces a choice between drift and a symbol overlap.
            for step in 1..20 {
                if let Some((x, y, dx, dy)) =
                    edge_label_anchor_at_fraction(edge, step as f32 / 20.0)
                {
                    for offset in [0.0, target_gap, -target_gap] {
                        let point = (x - dy * offset, y + dx * offset);
                        let point = if let Some(bound) = bounds {
                            clamp_label_center_to_bounds(
                                point,
                                label.width,
                                label.height,
                        ...
                        push_center_unique(&mut candidates, point);
                    }
                }
            }
            push_center_unique(&mut candidates, center);
            ...
            let current_static_overlap: f32 = current_label_rects[idx]
                .map(|rect| {
                    static_obstacles
                        .iter()
                        .map(|obstacle| overlap_area(&rect, obstacle))
            ...
            for cand in candidates {
                ...
                // Attachment polishing must not reintroduce node/head collisions
                // that the main placement pass has already removed.
                if overlap > current_static_overlap + LABEL_OVERLAP_WIDE_THRESHOLD {
                    continue;
                }
...
pub(super) fn polyline_rect_distance(points: &[(f32, f32)], rect: &Rect) -> f32 {
    if points.len() < 2 {
        return f32::INFINITY;
    }
    let mut best = f32::INFINITY;
...
fn build_label_obstacles(
    ...
    occupied.extend(
        edges
            .iter()
            .flat_map(|edge| crate::edge_geometry::endpoint_obstacles(edge, kind)),
    );
    occupied
}
...
fn label_penalties(
    ...
    for i in ctx.occupied_grid.query(&rect) {
        ...
        if ov > 0.0 {
            // A tiny overlap can erase an entire small endpoint symbol. Keep
            // collision avoidance ahead of the softer edge-distance preference.
            if i < ctx.node_obstacle_count {
                overlap += area * 4.0;
            }
...
mod tests {
    #[test]
    fn requirement_seed_labels_move_clear_of_endpoint_symbols() {
        let source = "requirementDiagram\nrequirement req1 {\nid: 1\ntext: Login\n}\nrequirement req2 {\nid: 2\ntext: Audit\n}\nreq1 - contains -> req2\nreq2 - satisfies -> req1";
        let graph = crate::parse_mermaid_strict(source).unwrap().graph;
        let layout = crate::compute_layout(
            &graph,
            &crate::Theme::modern(),
        ...
        let obstacles: Vec<_> = layout
            .edges
            .iter()
            .flat_map(|edge| crate::edge_geometry::endpoint_obstacles(edge, layout.kind))
            .collect();
        for edge in &layout.edges {
            let (Some(label), Some((x, y))) = (&edge.label, edge.label_anchor) else {
                continue;
            };
            let rect = (
                x - label.width / 2.0,
            ...
            for obstacle in &obstacles {
                assert_eq!(
                    super::overlap_area(&rect, obstacle),
                    0.0,
                    "label {} overlaps {obstacle:?}: {rect:?}",
                    label.lines.join(" ")
                );
            }
        }
    ...
    #[test]
    fn er_labels_avoid_endpoint_symbols_in_all_directions() {
