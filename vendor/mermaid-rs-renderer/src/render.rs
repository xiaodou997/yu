...
pub fn measure_svg_dimensions(
    ...
    let Some(title) = &layout.frontmatter_title else {
        return measure_base_svg_dimensions(layout, config, dimensions);
    };
    let base = measure_base_svg_dimensions(layout, config, None);
    let width = base
        .viewbox_width
        .max(title.text.width + TITLE_PADDING * 2.0);
    let height = base.viewbox_height + title.text.height + TITLE_PADDING * 2.0;
    let (mut target_width, mut target_height) =
        fit_dimensions_to_preferred_ratio(width, height, config.preferred_aspect_ratio);
...
pub fn render_svg_with_dimensions(
    ...
    let Some(title) = &layout.frontmatter_title else {
        return render_base_svg_with_dimensions(layout, theme, config, dimensions);
    };
    let base_dimensions = measure_base_svg_dimensions(layout, config, None);
    let size = measure_svg_dimensions(layout, config, dimensions);
    let base = render_base_svg_with_dimensions(layout, theme, config, None);
    // This is our own generated root, not arbitrary imported SVG. Retain all
    // diagram definitions and coordinates while composing a measured header.
    let content =
        &base[base.find('>').expect("generated SVG root") + 1..base.len() - "</svg>".len()];
    let mut svg = format!(
        "<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" width=\"{}\" height=\"{}\" viewBox=\"0 0 {} {}\"><rect width=\"{}\" height=\"{}\" fill=\"{}\"/>",
        size.width,
        size.height,
        size.viewbox_width,
        size.viewbox_height,
    ...
    for (index, line) in title.text.lines.iter().enumerate() {
        let baseline = TITLE_PADDING
            + title.font_size
            + index as f32 * title.font_size * config.label_line_height;
        svg.push_str(&format!("<text x=\"{}\" y=\"{}\" text-anchor=\"middle\" font-family=\"{}\" font-size=\"{}\" fill=\"{}\">{}</text>", size.viewbox_width*0.5,baseline,escape_xml(&theme.font_family),title.font_size,escape_xml(&theme.primary_text_color),escape_xml(line)));
    }
    let x = (size.viewbox_width - base_dimensions.viewbox_width) * 0.5 - base_dimensions.viewbox_x;
    let y = title.text.height + TITLE_PADDING * 2.0 - base_dimensions.viewbox_y;
    svg.push_str(&format!(
        "<g transform=\"translate({x},{y})\">{content}</g></svg>"
    ));
    svg
}

fn measure_base_svg_dimensions(
    layout: &Layout,
    config: &LayoutConfig,
    dimensions: Option<(f32, f32)>,
) -> SvgDimensions {
...
fn render_base_svg_with_dimensions(
    layout: &Layout,
    theme: &Theme,
    config: &LayoutConfig,
    dimensions: Option<(f32, f32)>,
    ...
    let SvgDimensions {
        ...
        viewbox_width,
        viewbox_height,
    } = measure_base_svg_dimensions(layout, config, dimensions);
    ...
    if layout.kind == crate::ir::DiagramKind::Architecture {
        svg.push_str(&render_architecture(layout, theme, config));
        svg.push_str("</svg>");
        return svg;
    }
    ...
    for subgraph in &layout.subgraphs {
        ...
        if is_state {
            ...
            if !label_empty {
                let (label_x, top, _, height) = subgraph.label_bounds(layout.kind, theme);
                let label_y = top + height / 2.0;
            ...
            if !label_empty {
                let (left, top, width, height) = subgraph.label_bounds(layout.kind, theme);
                let label_x = left + width / 2.0;
                let label_y = top + height / 2.0;
    ...
    if is_sequence {
        for (edge_idx, edge) in layout.edges.iter().enumerate() {
            ...
            svg.push_str(&edge_arrowheads_svg(
                edge,
                layout.kind,
                &stroke,
                stroke_width,
        ...
        for number in seq_data.map(|s| s.numbers.as_slice()).unwrap_or_default() {
            svg.push_str(&format!(
                "<rect x=\"{:.2}\" y=\"{:.2}\" width=\"{:.2}\" height=\"{:.2}\" rx=\"{:.2}\" fill=\"{}\" stroke=\"{}\" stroke-width=\"1\"/>",
...
// Native SVG primitives, shared by the top actor and its surviving footbox.
fn sequence_actor_body_svg(node: &crate::layout::NodeLayout, theme: &Theme) -> String {
    let cx = node.x + node.width / 2.0;
    let top = node.y + theme.font_size * 0.25;
    let bottom = node.y + node.height - node.label.height - theme.font_size * 0.6;
    let height = (bottom - top).max(1.0);
    let radius = height * 0.12;
    let neck = top + radius * 2.0;
    let shoulder = top + height * 0.36;
...
fn render_sankey(layout: &SankeyLayout, theme: &Theme, _config: &LayoutConfig) -> String {
    ...
    for (idx, node) in layout.nodes.iter().enumerate() {
        ...
        let value_y = first_y + label_font_size * 1.15;
        svg.push_str(&format!(
            "<text x=\"{x:.2}\" y=\"{first_y:.2}\" text-anchor=\"{text_anchor}\" font-size=\"{label_font_size:.1}\">{label}</text><text x=\"{x:.2}\" y=\"{value_y:.2}\" text-anchor=\"{text_anchor}\" font-size=\"{label_font_size:.1}\">{value}</text>"
        ));
    }
    svg.push_str("</g>");
...
fn render_requirement(layout: &Layout, theme: &Theme, config: &LayoutConfig) -> String {
    ...
    for (edge_idx, edge) in layout.edges.iter().enumerate() {
        ...
        let d = points_to_path(&edge.points);
        svg.push_str(&format!(
            "<path id=\"{edge_id}\" data-edge-id=\"{edge_id}\" d=\"{d}\" fill=\"none\" stroke=\"{stroke}\" stroke-width=\"{stroke_width}\"{dash} stroke-linecap=\"round\" stroke-linejoin=\"round\"/>"
        ));
            ...
            && let Some(point) = edge.points.first().copied()
        {
            let angle = crate::edge_geometry::edge_endpoint_angle(&edge.points, true);
            svg.push_str(&format!(
                r#"<g class="requirement-contains" transform="translate({:.2} {:.2}) rotate({angle:.2})"><circle cx="10" cy="0" r="9" fill="none" stroke="{stroke}" stroke-width="{stroke_width}"/><path d="M 1 0 H 19 M 10 -9 V 9" fill="none" stroke="{stroke}" stroke-width="{stroke_width}"/></g>"#,
                point.0, point.1));
        }
        if edge.arrow_end
            && let Some(point) = edge.points.last().copied()
        {
            let angle = crate::edge_geometry::edge_endpoint_angle(&edge.points, false);
            svg.push_str(&format!(
                r#"<g class="requirement-arrowhead" transform="translate({:.2} {:.2}) rotate({angle:.2})"><path d="M -20 -10 L 0 0 L -20 10" fill="none" stroke="{stroke}" stroke-width="{stroke_width}"/></g>"#,
                point.0, point.1));
        }
            ...
            && let Some((x, y)) = edge.label_anchor
        {
            ...
            if req.edge_label_background != "none" {
                ...
                let fill_opacity = if visible { 1.0 } else { 0.0 };
                svg.push_str(&format!(
    ...
    for node in layout.nodes.values() {
        ...
        let header_y = node.y + req.label_padding_y + theme.font_size;
        if header_count >= 1 {
...
fn render_architecture(layout: &Layout, theme: &Theme, _config: &LayoutConfig) -> String {
...
fn render_pie(pie: &PieData, theme: &Theme, config: &LayoutConfig) -> String {
    ...
    let mut total: f64 = pie.legend.iter().map(|s| f64::from(s.value.max(0.0))).sum();
    if total <= 0.0 {
        total = pie.slices.iter().map(|s| f64::from(s.value.max(0.0))).sum();
    }
    ...
    for slice in &pie.slices {
        ...
        let percent = f64::from(slice.value) / total * 100.0;
...
fn render_gantt(
    ...
    for &(x, width) in &layout.excluded_spans {
        svg.push_str(&format!("<rect class=\"excluded-day\" x=\"{x:.2}\" y=\"{:.2}\" width=\"{width:.2}\" height=\"{:.2}\" fill=\"{}\" fill-opacity=\"0.16\"/>", layout.chart_y, layout.chart_height, escape_xml(&theme.primary_text_color)));
    }
    ...
    for task in &layout.tasks {
        ...
        // Include the diamond’s mitered stroke in its row bounds, leaving a
        // visible gap between milestones on adjacent rows.
        let milestone_size = (bar_height * 0.6).min(
            (layout.row_height / 2.0 - task.border_width / std::f32::consts::SQRT_2 - 1.0).max(0.0),
        );
        let mut label_rendered_inside = false;
        if task.status.is_some_and(|flags| flags.milestone) {
            let size = milestone_size;
            let cx = task.x;
            let cy = row_center;
            let points = format!(
                "{:.2},{:.2} {:.2},{:.2} {:.2},{:.2} {:.2},{:.2}",
            ...
            svg.push_str(&format!(
                ...
                points, task.color, task.border_color, task.border_width
            ));
        } else {
            svg.push_str(&format!(
                "<rect x=\"{:.2}\" y=\"{:.2}\" width=\"{:.2}\" height=\"{:.2}\" rx=\"3\" fill=\"{}\" stroke=\"{}\" stroke-width=\"{}\"/>",
                task.x,
                bar_y,
                task.width,
                bar_height,
    ...
    if let Some((x, style)) = &layout.today_marker {
        svg.push_str(&format!("<line class=\"today-marker\" x1=\"{x:.2}\" x2=\"{x:.2}\" y1=\"{:.2}\" y2=\"{:.2}\" stroke=\"{}\" stroke-width=\"{}\" opacity=\"{}\"/>", layout.chart_y, layout.chart_y + layout.chart_height, escape_xml(&style.stroke), style.width, style.opacity));
    }
    svg
}
...
fn render_gitgraph_multiline_text(
    ...
    let lines = crate::layout::split_lines(text);
    if lines.is_empty() {
        return String::new();
    }
    ...
    for (idx, line) in lines.iter().enumerate() {
        let line_y = start_y + idx as f32 * line_height_px;
        out.push_str(&format!(
            "<text x=\"{x:.2}\" y=\"{line_y:.2}\" text-anchor=\"start\" font-family=\"{}\" font-size=\"{}\" fill=\"{}\">{}</text>",
            normalize_font_family(font_family), font_size, escape_xml(color), escape_xml(line)
        ));
    }
    out
}
...
fn text_block_svg_with_font_size_weight(
    ...
    for (idx, line) in label.lines.iter().enumerate() {
        let line_y = start_y + idx as f32 * line_height;
        let rendered = if is_divider_line(line) {
            String::new()
        } else {
            escape_xml(line)
        };
        // AppKit's SVG importer flattens positioned tspans. Absolute text
        // elements preserve the same baselines in both native and SVG viewers.
        text.push_str(&format!(
            "<text x=\"{x:.2}\" y=\"{line_y:.2}\" text-anchor=\"{anchor}\" font-family=\"{}\" font-size=\"{}\" fill=\"{}\"{weight_attr}>{rendered}</text>",
            normalize_font_family(&theme.font_family), font_size, fill
        ));
    }
...
fn render_c4_rel(rel: &C4RelLayout, conf: &crate::config::C4Config) -> String {
    ...
    // Keep the final curve control point for exact endpoint tangents.
    let points = if rel.waypoints.len() > 2 {
        let d = points_to_path(&rel.waypoints);
        svg.push_str(&format!(
            r#"<path fill="none" stroke-width="1" stroke="{}" d="{d}"/>"#,
            escape_xml(stroke)
        ));
        rel.waypoints.clone()
    } else if rel.control.is_none() {
        svg.push_str(&format!(r#"<line x1="{:.2}" y1="{:.2}" x2="{:.2}" y2="{:.2}" stroke-width="1" stroke="{}" fill="none"/>"#,
            rel.start.0, rel.start.1, rel.end.0, rel.end.1, escape_xml(stroke)));
        vec![rel.start, rel.end]
    } else {
        let control = rel.control.expect("quadratic relationship control point");
        svg.push_str(&format!(r#"<path fill="none" stroke-width="1" stroke="{}" d="M {:.2} {:.2} Q {:.2} {:.2} {:.2} {:.2}"/>"#,
            escape_xml(stroke), rel.start.0, rel.start.1, control.0, control.1, rel.end.0, rel.end.1));
        vec![rel.start, control, rel.end]
    };
    ...
    if matches!(
        ...
        svg.push_str(&arrowhead_svg(
            rel.start,
            crate::edge_geometry::edge_endpoint_angle(&points, true) + 180.0,
            &escape_xml(stroke),
            1.0,
        ));
    }
    ...
    let (mid_x, mid_y) = rel.label_center;
    svg.push_str(&c4_text_svg(
        mid_x,
        mid_y,
        &rel.label.lines,
    ...
    if let (Some(techn), Some((x, y))) = (&rel.techn, rel.techn_center) {
        svg.push_str(&c4_text_svg(
            x,
            y,
            &techn.lines,
...
fn c4_text_svg(
    ...
    for (idx, line) in lines.iter().enumerate() {
        ...
        // Use an explicit alphabetic baseline: AppKit does not apply
        // dominant-baseline="middle" to imported SVG text.
        let line_y = y + dy + font_size * 0.35;
        out.push_str(&format!(
            "<text x=\"{x:.2}\" y=\"{line_y:.2}\" fill=\"{}\" style=\"text-anchor: middle; font-size: {}px; font-weight: {}; font-family: {}\"{}>{}</text>",
            escape_xml(fill),
            font_size,
...
fn text_lines_svg(
    ...
    for (pos, (idx, line)) in lines.iter().enumerate() {
        ...
        text.push_str(&format!(
            "<text x=\"{x:.2}\" y=\"{line_y:.2}\" text-anchor=\"{anchor}\" font-family=\"{}\" font-size=\"{}\" fill=\"{}\"{weight}>{}</text>",
            normalize_font_family(&theme.font_family), theme.font_size, fill, escape_xml(line)
        ));
    }
    text
}
...
fn er_badge_svg(x: f32, y: f32, text: &str, width: f32, theme: &Theme) -> String {
    ...
    format!(
        r##"<rect x="{x:.2}" y="{top:.2}" width="{width:.2}" height="{height:.2}" rx="{radius:.2}" fill="{fill}"/><text x="{center:.2}" y="{baseline:.2}" text-anchor="middle" font-family="{family}" font-size="{size:.2}" font-weight="600" fill="#FFFFFF">{text}</text>"##,
        top = y - height / 2.0,
        radius = height / 2.0,
        center = x + width / 2.0,
        baseline = y + font_size * 0.26,
        family = escape_xml(&normalize_font_family(&theme.font_family)),
        size = font_size * 0.72,
        text = escape_xml(text),
    )
}
...
fn render_er_node(
    ...
    let Some(table) = &node.er_table else {
        return String::new();
    };
    let x = node.x;
    let y = node.y;
    ...
    let border = node
        ...
        .as_ref()
        .unwrap_or(&theme.primary_border_color);
    let body = node.style.fill.as_ref().unwrap_or(&theme.background);
    let mut svg = format!(
        r#"<g class="er-entity"><rect x="{x:.2}" y="{y:.2}" width="{w:.2}" height="{h:.2}" rx="6" fill="{body}" stroke="{border}" stroke-width="{stroke}"/><rect x="{x:.2}" y="{y:.2}" width="{w:.2}" height="{header:.2}" rx="6" fill="{fill}"/>"#,
        stroke = node.style.stroke_width.unwrap_or(1.2),
        header = table.header_height,
        fill = theme.cluster_background,
    );
    svg.push_str(&text_block_svg_anchor(
        x + w / 2.0,
        y + table.header_height / 2.0,
        &table.title,
        theme,
        config,
        "middle",
        Some(&theme.primary_text_color),
    ));
    let line = |x1: f32, y1: f32, x2: f32, y2: f32| {
        format!(
            r#"<line x1="{x1:.2}" y1="{y1:.2}" x2="{x2:.2}" y2="{y2:.2}" stroke="{}" stroke-width="1" stroke-opacity="0.6"/>"#,
            theme.cluster_border
        )
    };
    let mut origins = [x; 4];
    for i in 1..4 {
        origins[i] = origins[i - 1] + table.columns[i - 1];
    }
    if !table.rows.is_empty() {
        for i in 1..4 {
            if table.columns[i] > 0.0 {
                svg.push_str(&line(
                    origins[i],
    ...
    for row in &table.rows {
        svg.push_str(r#"<g class="er-attribute">"#);
        let center = y + row.top + row.height / 2.0;
        svg.push_str(&line(x, y + row.top, x + w, y + row.top));
        for (index, label) in [(0, &row.name), (1, &row.data_type), (3, &row.comment)] {
            if table.columns[index] > 0.0 {
                svg.push_str(&text_block_svg_anchor(
                    origins[index] + table.padding,
                    center,
                    label,
                    theme,
                    config,
        ...
        let mut key_x = origins[2] + table.padding;
        for (key, width) in &row.keys {
            svg.push_str(&er_badge_svg(key_x, center, key, *width, theme));
            key_x += width + theme.font_size * 0.4;
        }
        svg.push_str("</g>");
    }
    svg.push_str("</g>");
...
fn edge_decoration_svg(
    ...
    let shape = match decoration {
        ...
        crate::ir::EdgeDecoration::CrowsFootOne => format!(
            "<path d=\"M 4 -6 L 4 6 M 10 -6 L 10 6\" fill=\"none\" stroke=\"{}\" stroke-width=\"{}\"{join}/>",
            stroke, stroke_width
        ),
        crate::ir::EdgeDecoration::CrowsFootZeroOne => format!(
            "<g><circle cx=\"16\" cy=\"0\" r=\"4\" fill=\"none\" stroke=\"{}\" stroke-width=\"{}\"/><path d=\"M 4 -6 L 4 6\" fill=\"none\" stroke=\"{}\" stroke-width=\"{}\"{join}/></g>",
            stroke, stroke_width, stroke, stroke_width
        ),
        crate::ir::EdgeDecoration::CrowsFootMany => format!(
            "<path d=\"M 2 -6 L 10 0 L 2 6 M 16 -6 L 16 6\" fill=\"none\" stroke=\"{}\" stroke-width=\"{}\"{join}/>",
            stroke, stroke_width
        ),
        crate::ir::EdgeDecoration::CrowsFootZeroMany => format!(
            "<g><circle cx=\"16\" cy=\"0\" r=\"4\" fill=\"none\" stroke=\"{}\" stroke-width=\"{}\"/><path d=\"M 2 -6 L 10 0 L 2 6\" fill=\"none\" stroke=\"{}\" stroke-width=\"{}\"{join}/></g>",
            stroke, stroke_width, stroke, stroke_width
        ),
    };
...
fn edge_arrowheads_svg(
    ...
    let mut svg = String::new();
    for (at_start, enabled, arrow_kind) in [
        (true, edge.arrow_start, edge.arrow_start_kind),
        (false, edge.arrow_end, edge.arrow_end_kind),
    ] {
        if !enabled {
            continue;
        }
        ...
        let angle = crate::edge_geometry::edge_endpoint_angle(&edge.points, at_start)
            + if at_start { 180.0 } else { 0.0 };
        if kind == crate::ir::DiagramKind::State {
            svg.push_str(&state_arrowhead_svg(point, angle, stroke, stroke_width));
        } else if kind == crate::ir::DiagramKind::Class {
            let inset = crate::edge_geometry::arrowhead_inset(kind, arrow_kind);
            let (path, fill) = if arrow_kind == Some(crate::ir::EdgeArrowhead::OpenTriangle) {
                (format!("M {inset} 0 L 0 -6 L 0 6 Z"), "none")
            } else {
                (
                    format!("M {} -6 L {inset} 0 L {} 6", inset - 9.0, inset - 9.0),
                    "none",
                )
            };
            ...
            svg.push_str(&arrowhead_svg(point, angle, stroke, stroke_width));
        }
    }
    svg
}

// Preserve the state arrow's original concave shape without SVG markers,
// which AppKit's image importer does not paint. The endpoint is the tip;
// rotating around it keeps start/end arrows on the routed path in every direction.
fn state_arrowhead_svg(
    point: (f32, f32),
    angle_deg: f32,
    stroke: &str,
    stroke_width: f32,
    ...
    let (x, y) = point;
    format!(
        "<g class=\"state-arrowhead\" transform=\"translate({x:.2} {y:.2}) rotate({angle_deg:.2})\"><path d=\"M 0 0 L -10 6 L -5 0 L -10 -6 Z\" fill=\"{stroke}\" stroke=\"{stroke}\" stroke-width=\"{stroke_width}\" stroke-linejoin=\"round\"/></g>"
    )
}
...
fn arrowhead_svg(point: (f32, f32), angle_deg: f32, stroke: &str, stroke_width: f32) -> String {
    ...
    let join = " stroke-linejoin=\"round\" stroke-linecap=\"round\"";
    format!(
        "<g class=\"native-arrowhead\" transform=\"translate({x:.2} {y:.2}) rotate({angle_deg:.2})\"><polygon points=\"0,0 {neg_size:.2},{half:.2} {neg_size:.2},{neg_half:.2}\" fill=\"{stroke}\" stroke=\"{stroke}\" stroke-width=\"{stroke_width}\"{join}/></g>",
        neg_size = -size,
        half = half,
        neg_half = -half,
    )
...
fn shape_svg(node: &crate::layout::NodeLayout, theme: &Theme, config: &LayoutConfig) -> String {
    ...
    match node.shape {
        ...
        crate::ir::NodeShape::Actor => sequence_actor_body_svg(node, theme),
        ...
        crate::ir::NodeShape::Circle | crate::ir::NodeShape::DoubleCircle => {
            ...
            if node.shape == crate::ir::NodeShape::DoubleCircle {
                let r2 = r - 4.0;
                if r2 > 0.0 {
                    let inner_fill = if is_state_end {
                    ...
                    let inner_stroke = if is_state_end {
                        theme.line_color.as_str()
                    } else if label_empty {
                        theme.background.as_str()
                    } else {
...
mod tests {
    ...
    #[test]
    fn pie_slice_borders_use_configured_color_on_transparent_canvas() {
        for source in ["pie\n\"A\": 10", "pie\n\"A\": 10\n\"B\": 20"] {
            let parsed = crate::parse_mermaid_strict(source).expect("pie");
            let mut theme = crate::Theme::modern();
            theme.background = "none".into();
            theme.pie_stroke_color = "#abcdef".into();
            theme.pie_outer_stroke_color = "#123456".into();
            let config = LayoutConfig::default();
            let layout = crate::compute_layout(&parsed.graph, &theme, &config);
            let svg = render_svg(&layout, &theme, &config);
            assert!(svg.contains("stroke=\"#abcdef\""));
            assert!(svg.contains("stroke=\"#123456\""));
        }
    }

    #[test]
    fn er_cardinality_faces_the_edge_at_both_endpoints() {
        ...
        for decoration in [
            ...
            for tangent in [0.0, 90.0, 180.0, 270.0] {
                for at_start in [false, true] {
                    let svg = super::edge_decoration_svg(
                        (0.0, 0.0),
                        tangent,
                        decoration,
                        "black",
                    ...
                    let expected = if at_start { tangent } else { tangent + 180.0 };
                    assert!(svg.contains(&format!("rotate({expected:.2})")), "{svg}");
                    assert!(!svg.contains("cx=\"-4\""));
                }
            }
        }
    }
