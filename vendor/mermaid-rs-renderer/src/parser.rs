...
impl ParseOutput {
    /// Resolve source layout options without replacing the host's theme or
    /// unrelated layout settings. Root identity is resolved during parsing.
    pub fn apply_source_layout(&self, layout: &mut crate::config::LayoutConfig) -> Result<()> {
        if let Some(config) = &self.init_config {
            validate_source_config(config, self.graph.kind)?;
        }
        if self.graph.kind == DiagramKind::GitGraph {
            if let Some(config) = self
                .init_config
                .as_ref()
                .and_then(|config| config.get("gitGraph"))
            {
                layout.gitgraph.apply_source_options(config)?;
            }
        }
        Ok(())
    }
...
fn validate_source_config(value: &serde_json::Value, kind: DiagramKind) -> Result<()> {
    let object = value
        .as_object()
        .ok_or_else(|| anyhow::anyhow!("Mermaid init configuration must be an object"))?;
    for (key, value) in object {
        if key != "gitGraph" {
            bail!("unsupported native Mermaid init configuration: {key}");
        }
        if kind != DiagramKind::GitGraph {
            bail!("Mermaid gitGraph configuration requires a gitGraph diagram");
        }
        // Validate each directive, even if a later directive overrides it.
        crate::config::GitGraphConfig::default().apply_source_options(value)?;
    }
    Ok(())
}

fn merge_init_config(target: &mut Option<serde_json::Value>, incoming: serde_json::Value) {
    fn merge(target: &mut serde_json::Value, incoming: serde_json::Value) {
        if let (Some(existing), Some(values)) = (target.as_object_mut(), incoming.as_object()) {
...
/// Give quoted flowchart labels one logical line before line-oriented parsing.
/// Newlines become the same explicit line breaks used by native label layout.
/// Empty lines after the statement keep subsequent diagnostic line numbers stable.
/// This is a rendering projection only; callers retain the original source.
pub fn normalize_flowchart_multiline(input: &str) -> Result<std::borrow::Cow<'_, str>> {
    use std::borrow::Cow;
    if detect_diagram_kind(input) != Some(DiagramKind::Flowchart) {
    ...
    for (line_index, line) in input.lines().enumerate() {
        ...
        if quote.is_none() {
            ...
            if frontmatter || trimmed == "---" || trimmed.starts_with("%%") {
        ...
        while let Some((index, ch)) = chars.next() {
            if escaped {
                ...
                quote = Some(ch);
                quote_line = line_index + 1;
            }
            output.push(ch);
        }
        // A physical newline is not escaped by a trailing backslash. Keep the
        // backslash itself as label content, then preserve the line boundary.
        escaped = false;
        if quote.is_some() {
            output.push_str("<br/>");
            deferred_lines += 1;
    ...
    if quote.is_some() {
        bail!("Unclosed flowchart quoted label on line {quote_line}");
    }
    Ok(if changed {
        Cow::Owned(output)
    } else {
...
fn read_frontmatter(input: &str) -> Result<Frontmatter<'_>> {
    ...
    for line in input.split_inclusive('\n') {
        ...
        if line.trim_start().starts_with("%%") {
            saw_directive |= line.trim_start().starts_with("%%{");
            continue;
        }
        if line.trim() == "---" {
            if saw_directive {
                bail!("Mermaid frontmatter must precede init directives");
            }
            opening = Some(offset);
        }
        break;
    ...
    for line in input[start..].split_inclusive('\n') {
        count += 1;
        // An indented --- inside a YAML block scalar is literal title text.
        if line.trim_end() == "---" {
    ...
    let (end, body) = closing.ok_or_else(|| anyhow::anyhow!("Unterminated Mermaid frontmatter"))?;
    result.body = &input[body..];
    result.lines = count;
    let value: serde_yaml_ng::Value = serde_yaml_ng::from_str(&input[start..end])
        .map_err(|error| anyhow::anyhow!("invalid Mermaid YAML frontmatter: {error}"))?;
    if value.is_null() {
        if input[start..end]
            .lines()
        ...
        bail!("Mermaid frontmatter must be a mapping");
    }
    let map = value
        .as_mapping()
        .ok_or_else(|| anyhow::anyhow!("Mermaid frontmatter must be a mapping"))?;
    for (key, value) in map {
        let key = key
            .as_str()
            .ok_or_else(|| anyhow::anyhow!("Mermaid frontmatter keys must be strings"))?;
        match key {
            "title" | "displayMode" => {
                let text = value
                    .as_str()
                    .ok_or_else(|| anyhow::anyhow!("Mermaid frontmatter {key} must be a string"))?;
                if key == "title" {
                    result.title = Some(text.trim_end_matches('\n').to_owned());
                } else {
                    result.display_mode = Some(text.to_owned());
                }
            }
            "config" => {
                if !value.is_mapping() {
                    bail!("Mermaid frontmatter config must be a mapping");
                }
                result.config =
                    Some(serde_yaml_ng::from_value(value.clone()).map_err(|error| {
                        anyhow::anyhow!("invalid Mermaid frontmatter config: {error}")
                    })?);
            }
            _ => bail!("unsupported native Mermaid frontmatter field: {key}"),
        }
    }
    Ok(result)
}

fn prepared_frontmatter(frontmatter: &Frontmatter<'_>) -> String {
    let mut result = String::new();
    if let Some(config) = &frontmatter.config {
        result.push_str(&format!("%%{{init: {config}}}%%\n"));
    }
    // Keep physical line positions for the parser and host diagnostics.
    result.extend(std::iter::repeat_n(
        '\n',
        frontmatter
            .lines
            .saturating_sub(usize::from(frontmatter.config.is_some())),
    ));
    result.push_str(frontmatter.body);
    result
}

/// Shared source projection for native host validation. The caller retains the
/// original document; title and display metadata are applied by the parser.
pub fn prepare_frontmatter_source(input: &str) -> Result<String> {
...
fn parse_with_consumption_checks(
    ...
    validate_init_directives(input, strict)?;
    let Some(kind) = detect_diagram_kind(input) else {
        bail!("unknown or missing Mermaid diagram header");
    };
    let mut parsed = match kind {
        DiagramKind::Class => parse_class_diagram(input, strict),
        DiagramKind::State => parse_state_diagram(input, strict),
        DiagramKind::Sequence => parse_sequence_diagram(input, strict),
        DiagramKind::Er => parse_er_diagram(input, strict),
        ...
        DiagramKind::Gantt => parse_gantt_diagram(input, reference_day),
        ...
        DiagramKind::Flowchart => parse_flowchart(input, strict),
    }?;
    if let Some(mode) = frontmatter.display_mode {
        if kind != DiagramKind::Gantt || mode != "compact" {
            bail!("unsupported Mermaid displayMode for this diagram: {mode}");
        }
        parsed.graph.gantt_display_mode = Some(mode);
    }
    if let Some(title) = frontmatter.title.filter(|title| !title.is_empty()) {
        let native_title = match kind {
            DiagramKind::Pie => Some(&mut parsed.graph.pie_title),
            DiagramKind::Gantt => Some(&mut parsed.graph.gantt_title),
            DiagramKind::Journey => Some(&mut parsed.graph.journey_title),
            DiagramKind::Timeline => Some(&mut parsed.graph.timeline.title),
            DiagramKind::Quadrant => Some(&mut parsed.graph.quadrant.title),
            DiagramKind::XYChart => Some(&mut parsed.graph.xychart.title),
            _ => None,
        };
        if let Some(existing) = native_title {
            if existing.is_none() {
                *existing = Some(title);
            }
        } else {
            parsed.graph.frontmatter_title = Some(title);
        }
    }
    Ok(parsed)
}

fn is_state_note_header(line: &str) -> bool {
    let line = line.trim().to_ascii_lowercase();
    (line.starts_with("note right of ") || line.starts_with("note left of ")) && !line.contains(':')
}

// Generic validation must not execute directives or interpret arrows inside
// a note. Keep physical lines so diagnostic positions remain source-based.
pub(crate) fn mask_state_note_bodies(input: &str) -> std::borrow::Cow<'_, str> {
    use std::borrow::Cow;
    if detect_diagram_kind(input) != Some(DiagramKind::State) {
    ...
    let mut changed = false;
    for line in input.split_inclusive('\n') {
        if in_note && !line.trim().eq_ignore_ascii_case("end note") {
            if line.ends_with('\n') {
                output.push('\n');
            }
            changed = true;
        ...
        if in_note {
            in_note = false;
        } else if is_state_note_header(line) {
            in_note = true;
        }
        output.push_str(line);
    }
...
fn validate_init_directives(input: &str, strict: bool) -> Result<()> {
    let masked = mask_state_note_bodies(input);
    ...
    for raw_line in input.lines() {
        ...
        if let Some(value) = parse_init_directive(trimmed_line)?
            ...
            && let Some(kind) = kind
        {
            validate_source_config(&value, kind)?;
        }
    }
    Ok(())
}
...
fn preprocess_input(input: &str) -> Result<(Vec<String>, Option<serde_json::Value>)> {
    ...
    let state_diagram = detect_diagram_kind(input) == Some(DiagramKind::State);
    let mut in_note = false;
    ...
    for raw_line in input.lines() {
        ...
        if in_note {
            ...
            if trimmed_line.eq_ignore_ascii_case("end note") {
                in_note = false;
            }
            continue;
        }
        ...
        if let Some(value) = parse_init_directive(trimmed_line)? {
            merge_init_config(&mut init_config, value);
            continue;
        }
        if trimmed_line.starts_with("%%") {
            continue;
        ...
        in_note = state_diagram && is_state_note_header(&without_comment);
...
fn parse_flowchart(input: &str, strict: bool) -> Result<ParseOutput> {
    ...
    for raw_line in lines {
        for line in split_statements(&raw_line) {
            ...
            if strict {
                let (base, _) = split_inline_classes(&line);
                let shaped =
                    split_id_label(&base).is_some() || split_asymmetric_label(&base).is_some();
                if !shaped && (base.is_empty() || base.chars().any(char::is_whitespace)) {
                    bail!("unrecognized flowchart statement: {line}");
                }
            }
...
fn parse_sequence_message(
    ...
    for &token in SEQUENCE_ARROWS {
        let Some((left, right)) = participants.split_once(token) else {
            continue;
        };
        let left = left.trim();
        ...
        let half = match token {
            r"-|\" | r"--|\" | "/|-" | "/|--" => Some(crate::ir::EdgeArrowhead::HalfTop),
            "-|/" | "--|/" | r"\|-" | r"\|--" => Some(crate::ir::EdgeArrowhead::HalfBottom),
            r"-\\" | r"--\\" | "//-" | "//--" => Some(crate::ir::EdgeArrowhead::StickTop),
            "-//" | "--//" | r"\\-" | r"\\--" => Some(crate::ir::EdgeArrowhead::StickBottom),
            _ => None,
        };
        let reverse = token == "<-" || token == "<--" || token.starts_with(['/', '\\']);
        let (from, to) = if reverse {
            (right, left)
        } else {
            (left, right)
        };
        let arrow_end = half.is_some() || token.ends_with(">>") || token.ends_with(')');
        let meta = EdgeMeta {
            directed: true,
            arrow_start: token.starts_with("<<"),
            arrow_end,
            arrow_start_kind: None,
            arrow_end_kind: if half.is_some() {
                half
            } else if token.ends_with(')') {
                Some(crate::ir::EdgeArrowhead::OpenArrow)
            } else {
                None
            },
            start_decoration: None,
            end_decoration: if token.ends_with('x') {
                Some(crate::ir::EdgeDecoration::Cross)
            } else {
                None
            },
            style: if token.contains("--") {
                crate::ir::EdgeStyle::Dotted
            } else {
                crate::ir::EdgeStyle::Solid
            },
...
fn parse_class_diagram(input: &str, strict: bool) -> Result<ParseOutput> {
    let mut graph = Graph::new();
    ...
    for raw_line in lines {
            ...
            if let Some(end_idx) = line.find('}') {
                if strict
                    && !line[end_idx + 1..]
                        ...
                        .is_empty()
                {
                    bail!("unexpected content after class body: {line}");
                }
        ...
        if strict {
            bail!("unrecognized class diagram statement: {line}");
        }
    }
    if strict && current_class.is_some() {
        bail!("unclosed class body");
    }
...
fn parse_er_entity_name(input: &str) -> Option<ErEntityName> {
    let name = |text: &str| -> Option<String> {
        let text = text.trim();
        if is_diagram_identifier(text) {
            Some(text.to_owned())
        } else {
            let decoded: String = json5::from_str(text).ok()?;
            (!decoded.is_empty() && !decoded.contains(['\r', '\n'])).then_some(decoded)
        }
    };
    let input = input.trim();
    if let Some(open) = find_unquoted_delimiter(input, '[') {
        let rest = &input[open + 1..];
        let close = find_unquoted_delimiter(rest, ']')?;
...
// Relationship punctuation inside quoted IDs or aliases is literal text.
fn find_er_separator(input: &str, token: &str) -> Option<usize> {
    let mut quote = None;
    let mut escaped = false;
...
fn parse_er_relation_line(
    ...
    let (relation_part, label) = if let Some(index) = find_er_separator(line, ":") {
    ...
    let (sep, style) = if let Some(idx) = find_er_separator(relation_part, "--") {
        (idx, crate::ir::EdgeStyle::Solid)
    } else if let Some(idx) = find_er_separator(relation_part, "..") {
    ...
    let left_id = parse_er_entity_name(&left_entity)?;
    let right_id = parse_er_entity_name(&right_entity)?;
...
fn is_diagram_identifier(input: &str) -> bool {
    !input.is_empty()
        && input
            .chars()
...
fn find_unquoted_delimiter(input: &str, target: char) -> Option<usize> {
    let mut quote = None;
    let mut escaped = false;
    for (index, ch) in input.char_indices() {
        if escaped {
...
fn parse_er_diagram(input: &str, strict: bool) -> Result<ParseOutput> {
    let mut graph = Graph::new();
    ...
    for raw_line in lines {
        ...
        if let Some(active) = current_entity.clone() {
            if let Some(end_idx) = find_unquoted_delimiter(line, '}') {
                if strict
                    && !line[end_idx + 1..]
                        .trim()
                        .trim_end_matches(';')
                        ...
                        .is_empty()
                {
                    bail!("unexpected content after ER entity body: {line}");
                }
                let fragment = line[..end_idx].trim();
                if !fragment.is_empty() {
                    members
        ...
        if let Some(open_idx) = find_unquoted_delimiter(line, '{') {
            ...
            if !name.is_empty() {
                ...
                if let Some(close_idx) = find_unquoted_delimiter(tail, '}') {
                    if strict
                        && !tail[close_idx + 1..]
                            .trim()
                            .trim_end_matches(';')
    ...
    if strict && current_entity.is_some() {
        bail!("unclosed ER entity body");
    }
    for (id, node) in graph.nodes.iter_mut() {
        let mut lines = Vec::new();
        ...
        {
            lines.push("---".to_string());
            match crate::er::split_attributes(&attrs.join("\n")) {
                Ok(attributes) => lines.extend(attributes),
                Err(error) if strict => return Err(error),
                Err(_) => lines.extend(attrs.clone()),
            }
        }
        node.label = lines.join("\n");
    }
...
/// Parse one quoted pie entry using the same rules as native rendering.
/// Returns no entry for malformed labels or non-finite/negative values.
pub fn parse_quoted_pie_entry(line: &str) -> Option<(String, f32)> {
    let line = line.trim();
    let mut labels = serde_json::Deserializer::from_str(line).into_iter::<String>();
    let label = labels.next()?.ok()?;
    if label.is_empty() {
        return None;
    }
    let value = line[labels.byte_offset()..]
        .trim_start()
        .strip_prefix(':')?;
    let value = strip_trailing_comment(value).parse::<f32>().ok()?;
    (value.is_finite() && value >= 0.0).then_some((label, value))
}

fn parse_pie_slice_line(line: &str) -> Option<(String, f32)> {
    if line.trim_start().starts_with('"') {
        return parse_quoted_pie_entry(line);
    }
...
fn parse_gantt_diagram(input: &str, reference_day: Option<i32>) -> Result<ParseOutput> {
    ...
    graph.direction = Direction::LeftRight;
    let (lines, init_config) = preprocess_input(input)?;
    let mut date_format = "YYYY-MM-DD";
    for line in &lines {
        let line = line.trim();
        let mut words = line.splitn(2, char::is_whitespace);
        if words.next() == Some("dateFormat") {
            let format = words.next().unwrap_or("").trim();
            if !crate::gantt_time::supports_date_format(format) {
                bail!("Unsupported native Gantt dateFormat: {format}");
            }
            date_format = format;
        }
    }
    ...
    for raw_line in lines {
        ...
        if lower == "axisformat" || lower == "tickinterval" {
            bail!("Invalid Gantt axis directive: {line}");
        }
        ...
        if lower == "todaymarker" {
            ...
            graph.gantt_today_marker = Some(style);
            continue;
        }
        if lower == "section" {
            let label = data;
...
fn parse_gantt_task_meta(
    ...
    while let Some(next) = parts.first().and_then(|s| gantt_status_from_token(s)) {
        ...
        flags.done |= next.done;
        flags.active |= next.active;
        flags.critical |= next.critical;
...
fn gantt_status_from_token(token: &str) -> Option<crate::ir::GanttStatus> {
    let mut flags = crate::ir::GanttStatus::default();
    match token {
        "done" => flags.done = true,
        "active" => flags.active = true,
        "crit" => flags.critical = true,
...
fn looks_like_duration(token: &str) -> bool {
    crate::gantt_time::duration_days(token).is_some()
}
...
fn parse_gitgraph_diagram(input: &str) -> Result<ParseOutput> {
    ...
    let mut git_options = crate::config::GitGraphConfig::default();
    if let Some(config) = init_config
        .as_ref()
        .and_then(|config| config.get("gitGraph"))
    {
        git_options.apply_source_options(config)?;
    }
    let main_branch = git_options.main_branch_name;
    let main_order = git_options.main_branch_order;
    ...
    for raw_line in lines {
        ...
        if lower == "gitgraph" || lower == "gitgraph:" {
            let suffix = arguments.trim().trim_end_matches(':').trim();
            if !suffix.is_empty() {
                graph.direction = parse_gitgraph_direction(suffix)
                    .ok_or_else(|| anyhow::anyhow!("unsupported gitGraph header: {line}"))?;
            }
            continue;
        }
        ...
        if lower == "branch" {
            let (name, suffix) = parse_gitgraph_target(arguments)?;
            let name = name.as_str();
            if branch_heads.contains_key(name) {
                bail!("gitGraph branch already exists: {name}");
            }
            if !name.is_empty() {
                let order = if suffix.is_empty() {
                    None
                } else {
                    let value = suffix.strip_prefix("order:").ok_or_else(|| {
                        anyhow::anyhow!("unsupported gitGraph branch suffix: {suffix}")
                    })?;
                    Some(
                        value
                            .trim()
                            ...
                            .ok_or_else(|| {
                                anyhow::anyhow!("gitGraph branch order must be a finite number")
                            })?,
                    )
                };
        ...
        if lower == "checkout" || lower == "switch" {
            let (name, suffix) = parse_gitgraph_target(arguments)?;
            if !suffix.is_empty() {
                bail!("unexpected gitGraph checkout suffix: {suffix}");
            }
            let name = name.as_str();
            if !branch_heads.contains_key(name) {
                bail!("gitGraph checkout branch does not exist: {name}");
            }
            current_branch = name.to_string();
            continue;
        }
        if lower == "merge" {
            let (from_branch, suffix) = parse_gitgraph_target(arguments)?;
            let attrs = parse_gitgraph_commit_attributes(suffix)?;
            let from_branch = from_branch.as_str();
            if from_branch == current_branch {
                bail!("gitGraph cannot merge a branch with itself: {from_branch}");
            }
            let from_head = branch_heads
                .get(from_branch)
                .ok_or_else(|| {
                    anyhow::anyhow!("gitGraph merge branch does not exist: {from_branch}")
                })?
                .clone();
            let current_head = branch_heads.get(&current_branch).cloned().flatten();
            if from_head.is_none() || current_head.is_none() {
                bail!("gitGraph merge requires a commit on both branches");
            }
            ...
            let (id, custom_id) = attrs.id.map(|value| (value, true)).unwrap_or_else(|| {
                let hex = rng.next_hex(7);
                (format!("{commit_seq}-{hex}"), false)
            });
            if !commit_ids.insert(id.clone()) {
                bail!("duplicate gitGraph commit id: {id}");
            }
            let tags = attrs.tags;
            let custom_type = attrs.commit_type;
        ...
        if lower == "commit" {
            let attrs = parse_gitgraph_commit_attributes(arguments)?;
            let (id, custom_id) = attrs.id.map(|value| (value, true)).unwrap_or_else(|| {
                let hex = rng.next_hex(7);
                (format!("{commit_seq}-{hex}"), false)
            });
            ...
            let tags = attrs.tags;
            let commit_type = attrs
                .commit_type
                .unwrap_or(crate::ir::GitGraphCommitType::Normal);
        ...
        bail!("unsupported gitGraph statement: {line}");
    }
...
#[derive(Default)]
struct GitGraphCommitAttributes {
    id: Option<String>,
    commit_type: Option<crate::ir::GitGraphCommitType>,
    tags: Vec<String>,
}

// Consume each attribute exactly once. Quoted text is never searched for keys.
fn parse_gitgraph_commit_attributes(mut input: &str) -> Result<GitGraphCommitAttributes> {
    let mut attrs = GitGraphCommitAttributes::default();
    let mut seen = HashSet::new();
    while !input.trim().is_empty() {
        ...
        let Some(value) = rest.strip_prefix(':') else {
            bail!("expected gitGraph attribute key:value: {input}");
        };
        if !matches!(key.as_str(), "id" | "type" | "tag") {
            bail!("unsupported gitGraph commit attribute: {key}");
        }
        if key != "tag" && !seen.insert(key.clone()) {
            bail!("duplicate gitGraph commit attribute: {key}");
        }
        let (value, suffix) = parse_gitgraph_target(value)?;
        match key.as_str() {
            "id" => attrs.id = Some(value),
            "tag" => attrs.tags.push(value),
            "type" => {
                attrs.commit_type = Some(match value.to_ascii_uppercase().as_str() {
                    "NORMAL" => crate::ir::GitGraphCommitType::Normal,
                    "REVERSE" => crate::ir::GitGraphCommitType::Reverse,
                    "HIGHLIGHT" => crate::ir::GitGraphCommitType::Highlight,
                    _ => bail!("unsupported gitGraph commit type: {value}"),
                })
            }
            _ => unreachable!(),
        }
...
// Separate branch identity from command attributes before updating history.
fn parse_gitgraph_target(input: &str) -> Result<(String, &str)> {
    let input = input.trim();
    let Some(first) = input.chars().next() else {
        bail!("missing gitGraph branch name");
    };
    if first == '"' || first == '\'' {
        let mut escaped = false;
        for (index, ch) in input.char_indices().skip(1) {
            ...
            if ch == first {
                let name: String = json5::from_str(&input[..index + 1]).map_err(|error| {
                    anyhow::anyhow!("invalid quoted gitGraph branch name: {error}")
                })?;
                if name.trim().is_empty() {
                    bail!("empty gitGraph branch name");
                }
                let suffix = &input[index + 1..];
                if !suffix.is_empty() && !suffix.starts_with(char::is_whitespace) {
                    bail!("expected whitespace after quoted gitGraph branch name");
                }
                return Ok((name, suffix.trim()));
            }
        }
        bail!("unclosed quoted gitGraph branch name");
    }
    let end = input.find(char::is_whitespace).unwrap_or(input.len());
    Ok((input[..end].to_string(), input[end..].trim()))
}
...
fn parse_state_diagram(input: &str, strict: bool) -> Result<ParseOutput> {
    let mut graph = Graph::new();
    ...
    while let Some(raw_line) = pending.pop_front() {
        for raw_statement in split_statements(&raw_line) {
            let mut raw_statement = raw_statement;
            let raw_line = raw_statement.trim();
            if is_state_note_header(raw_line) {
                let mut body = Vec::new();
                let mut closed = false;
                while let Some(note_line) = pending.pop_front() {
                    if note_line.trim().eq_ignore_ascii_case("end note") {
                        closed = true;
                        break;
                    }
                    body.push(note_line);
                ...
                if !closed {
                    bail!("unclosed state note: {raw_line}");
                }
                raw_statement = format!("{raw_line}: {}", body.join("\n"));
            }
            let raw_line = raw_statement.trim();
            if raw_line.is_empty() {
                continue;
            }
            if let Some((position, target_raw, label)) = parse_state_note(raw_line) {
            ...
            if line == "}" {
                if strict && composite_stack.is_empty() {
                    bail!("unmatched closing state brace");
                }
            ...
            if line == "--" {
                if strict && composite_stack.is_empty() {
                    bail!("state concurrency separator requires a composite state");
                }
            ...
            if let Some((id, classes)) = parse_state_simple(line)
                .or_else(|| is_diagram_identifier(line).then(|| (line.to_string(), Vec::new())))
            {
            ...
            if strict {
                bail!("unrecognized state statement: {line}");
            }
        }
    }
    if strict && !composite_stack.is_empty() {
        bail!("unclosed composite state");
    }
...
fn parse_sequence_number(token: &str) -> Result<u64> {
    ...
    {
        bail!(
            "invalid autonumber value (expected a nonnegative number with at most two decimals): {token}"
        );
    }
    ...
    whole
        ...
        .ok_or_else(|| anyhow::anyhow!("autonumber value exceeds supported range: {token}"))
}

fn parse_sequence_diagram(input: &str, strict: bool) -> Result<ParseOutput> {
    let mut graph = Graph::new();
    ...
    let mut numbering: Option<(Option<u64>, u64)> = None;
    ...
    for raw_line in lines {
        ...
        if lower.starts_with("create ") {
            let (id, label, shape) = parse_sequence_participant(line[7..].trim())
                .ok_or_else(|| anyhow::anyhow!("invalid participant creation: {line}"))?;
        ...
        if lower.split_whitespace().next() == Some("else") {
            if strict
                && !open_frames
                    .last()
                    .is_some_and(|frame| frame.kind == crate::ir::SequenceFrameKind::Alt)
            {
                bail!("sequence else requires an active alt frame: {line}");
            }
        ...
        if lower.split_whitespace().next() == Some("and") {
            if strict
                && !open_frames
                    .last()
                    .is_some_and(|frame| frame.kind == crate::ir::SequenceFrameKind::Par)
            {
                bail!("sequence and requires an active par frame: {line}");
            }
            if let Some(frame) = open_frames.last_mut()
        ...
        if lower.split_whitespace().next() == Some("option") {
            if strict
                && !open_frames
                    .last()
                    .is_some_and(|frame| frame.kind == crate::ir::SequenceFrameKind::Critical)
            {
                bail!("sequence option requires an active critical frame: {line}");
            }
            if let Some(frame) = open_frames.last_mut()
        ...
        if lower.split_whitespace().next() == Some("autonumber") {
            let parts = line.split_whitespace().collect::<Vec<_>>();
            numbering = match parts.as_slice() {
                [_] => Some((Some(100), 100)),
                [_, mode]
                    if matches!(
                ...
                [_, start] => Some((Some(parse_sequence_number(start)?), 100)),
                [_, start, step] => Some((
                    Some(parse_sequence_number(start)?),
                    parse_sequence_number(step)?,
                )),
                _ => bail!("invalid autonumber command: {line}"),
            };
            continue;
        }
        ...
        if let Some((from, to, label, meta, activation, connection)) = parse_sequence_message(line)
        {
            ...
            ensure_sequence_node(&mut graph, &labels, &from, None);
            ensure_sequence_node(&mut graph, &labels, &to, None);
            let number = if let Some((next, step)) = &mut numbering {
                let value = next.ok_or_else(|| {
                    anyhow::anyhow!("autonumber sequence exceeds supported range")
                })?;
                *next = value.checked_add(*step);
                Some(crate::ir::SequenceNumber(value))
            } else {
                None
            };
            ...
            bail!("unrecognized sequence diagram statement: {line}");
        }
    }
...
mod tests {
    ...
    #[test]
    fn flowchart_multiline_projection_keeps_labels_and_following_line_numbers() {
        let source = "---\r\ntitle: \"Quoted title\"\r\n---\r\nflowchart TD\r\n%% untouched \" comment\r\nA[\"First\r\n\r\n%% literal\r\nend\"] --> B[Done]\r\nB --> C";
        let normalized = normalize_flowchart_multiline(source).unwrap();
        assert!(normalized.contains("A[\"First<br/><br/>%% literal<br/>end\"] --> B[Done]"));
        assert_eq!(
            normalized
                .lines()
                .position(|line| line.starts_with("B --> C")),
            Some(9)
        );
        ...
        assert_eq!(
            ...
            "First<br/><br/>%% literal<br/>end"
        );
        assert!(
            normalize_flowchart_multiline("flowchart TD\nA[\"one\ntwo")
                .unwrap_err()
                .to_string()
                .contains("line 2")
        );
        ...
        assert!(matches!(
            normalize_flowchart_multiline(plain).unwrap(),
            std::borrow::Cow::Borrowed(_)
        ));
        let sequence = "sequenceDiagram\nA->>B: \"ordinary message";
        assert!(matches!(
            normalize_flowchart_multiline(sequence).unwrap(),
            std::borrow::Cow::Borrowed(_)
        ));
    }
...
A["foo & bar"] & B --> C"#;
    ...
    fn preprocess_keep_indent_skips_yaml_frontmatter() {
        let input = "---\ntitle: Roadmap\ndisplayMode: compact\n---\nmindmap\n  root\n    child";
        let prepared = prepare_frontmatter_source(input).unwrap();
        let (lines, init_config) = preprocess_input_keep_indent(&prepared).unwrap();
  ...
  req1 - contains -> webapp"##;
    ...
    fn parse_sequence_autonumber_off() {
        let input = "sequenceDiagram\nautonumber off\nA->>B: ping";
        let parsed = parse_mermaid(input).unwrap();
        assert_eq!(parsed.graph.sequence_message_numbers, vec![None]);
    }
