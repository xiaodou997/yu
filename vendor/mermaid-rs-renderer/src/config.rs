...
impl GitGraphConfig {
    /// Apply the native-supported Mermaid source options atomically. Unknown
    /// keys and wrong types must not turn into successful default rendering.
    pub fn apply_source_options(&mut self, value: &serde_json::Value) -> anyhow::Result<()> {
        let object = value
            .as_object()
            .ok_or_else(|| anyhow::anyhow!("gitGraph configuration must be an object"))?;
        let mut resolved = self.clone();
        for (key, value) in object {
            let boolean = || {
                value
                    .as_bool()
                    .ok_or_else(|| anyhow::anyhow!("gitGraph.{key} must be a boolean"))
            };
            let number = || {
                value
                    .as_f64()
                    ...
                    .ok_or_else(|| anyhow::anyhow!("gitGraph.{key} must be a finite number"))
            };
            match key.as_str() {
                "mainBranchName" => {
                    resolved.main_branch_name = value
                        .as_str()
                        .filter(|name| !name.trim().is_empty())
                        .ok_or_else(|| {
                            anyhow::anyhow!("gitGraph.mainBranchName must be a nonempty string")
                        })?
                        .to_string()
                }
                "mainBranchOrder" => resolved.main_branch_order = number()?,
                "showBranches" => resolved.show_branches = boolean()?,
                "showCommitLabel" => resolved.show_commit_label = boolean()?,
                "rotateCommitLabel" => resolved.rotate_commit_label = boolean()?,
                "parallelCommits" => resolved.parallel_commits = boolean()?,
                "useMaxWidth" => resolved.use_max_width = boolean()?,
                "diagramPadding" => {
                    let padding = number()?;
                    if padding < 0.0 {
                        anyhow::bail!("gitGraph.diagramPadding must be nonnegative");
                    }
                    resolved.diagram_padding = padding;
                }
                _ => anyhow::bail!("Unsupported native gitGraph configuration option: {key}"),
            }
        }
        *self = resolved;
        Ok(())
