...
/// ```
pub fn render_with_options(input: &str, mut options: RenderOptions) -> anyhow::Result<String> {
    let parsed = parse_mermaid_strict(input)?;
    parsed.apply_source_layout(&mut options.layout)?;
...
pub fn parse_mermaid_strict(input: &str) -> Result<ParseOutput, ParseError> {
    parse_mermaid_strict_at(input, None)
}

/// Parse with an explicit local Gregorian reference day (days since 1970-01-01).
pub fn parse_mermaid_strict_at(input: &str, reference_day: Option<i32>) -> Result<ParseOutput, ParseError> {
    let prepared =
        parser::prepare_frontmatter_source(input).map_err(|err| ParseError::UnexpectedToken {
            line: 1,
            col: 1,
            found: "frontmatter".into(),
            expected: err.to_string(),
        })?;
    let normalized = parser::normalize_flowchart_multiline(&prepared).map_err(|err| {
        ParseError::UnexpectedToken {
            line: 1,
            col: 1,
            found: "flowchart label".to_string(),
            expected: err.to_string(),
        }
    })?;
    validator::validate(normalized.as_ref())?;
    parser::parse_mermaid_checked(input, reference_day).map_err(|err| ParseError::UnexpectedToken {
        line: 1,
        col: 1,
        found: input
            .lines()
...
pub fn render_strict(input: &str, mut options: RenderOptions) -> Result<String, ParseError> {
    let parsed = parse_mermaid_strict(input)?;
    parsed
        .apply_source_layout(&mut options.layout)
        .map_err(|error| ParseError::UnexpectedToken {
            line: 1,
            col: 1,
            found: "configuration".into(),
