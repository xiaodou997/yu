...
8. `AtxHeading`、`BlockQuote` 和普通 `ListItem` 必须返回稳定的 `Heading`、`BlockQuote`、`List`
...
8a. GFM table 必须保持 `Paragraph` parser block kind，同时返回 source-backed `TableProjection`。
    ...
    内容编辑则必须重新构建 projection。projection 只能保留 header/body cell 内容的
    source-backed visual runs；pipe、cell 周围空白、row line ending 和 parser-owned delimiter
    physical row 必须是 zero-width hidden ranges。`TableLayoutSnapshot` 只能用这些 ranges 和
    外部 metrics/shaper 生成可见 cell geometry；测量必须消费 projection visible runs，不能复制
    cell 文本或把 delimiter 伪造成第二份 document model。`LayoutSnapshot` 的 visible cluster/glyph
    必须按 cell source range 重定位到同一 Revision 的 column/row/alignment，且 cell-aware
    caret/hit-test 在隐藏结构边界只能返回可见 cell source boundary，内部字符仍遵守 projection
    bias。native hit-test 必须返回同一 Revision 的
    ...
    不得进入 canonical layout cache。`yu_storage_session_table_layout_cells_with_resize` 必须
...
6. 活动 composition 必须通过 `EditorDocument::composition_block_range` 得到受影响的半开 block
   ...
   后续 block 清除其被替换的 source。transient block 高度只写入当前 viewport working state，不得
...
21. visual retained coverage 已验证当前 Revision、command mask、block mask 与 viewport blocks 后，
    ...
    为零的 image 必须保留 projection 已有的 alt-label glyph；unknown 且 fingerprint 非零或未知
    status 必须让 coverage incomplete 并整页 fail-closed，不能把未分类状态伪装成 retained coverage。
22. macOS production presentation 只能在 complete retained coverage 下选择完整 Rust surface，或在
    incomplete/stale/native failure 下选择完整 TextKit source fallback；不得把局部 TextKit glyph
    range 绘制在 accepted Metal frame 上。TextKit 可以继续拥有 input、IME、clipboard 与
    Accessibility 状态，但这些宿主职责不得隐式产生第二套 production glyph owner。
