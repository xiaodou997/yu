...
当前口径：CommonMark 0.31.2 的 652 条用例中 **642 条逐字节通过（98.47%）**，
其余 10 条分属下面三条偏差。**F3 已经关掉**（S7 第六刀），编号退休不再复用，
关掉它花了什么写在下面。
...
| F4 | 文档首行 `---` 启动 YAML front matter，下一行独立 `---` 或 `...` 关闭；未闭合时内容仍作为可编辑元信息源码 | 第四组明确要求 front matter；防止 YAML 中的标题、公式与列表被误当正文。首行水平线可使用 `***` | 96, 98 | 产品扩展，保留 |
