...
5. 本 ADR 只定义 source/cache/native metadata 边界。ImageIO worker、Metal RGBA texture、
   图片 Scene primitive 与未 ready fallback 已在 ADR 0142 作为资源级纵向切片实现；图片在
   block layout 中的 placement/hit-test 仍不属于本 ADR。
...
- `--visual-image-self-check` 验证 inline/reference image 的 UTF-16 ranges、fingerprint 和
  stale Revision rejection；ADR 0142 的 ImageIO/Metal resource tests 验证真实资源边界，但
  两者都不会假装图片已经自动进入产品窗口。
