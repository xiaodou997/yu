...
- **截图又抓到一个既有缺陷（没有在这一刀里修）**：嵌套的**任务项**不缩进。
  ...
  它因此**变得显眼**：同一层的列表项缩进了，任务项没有。已登记，见「还没做的」。
...
- **新发现（第十一刀 b 的截图抓到的）：嵌套的任务项不缩进。** `  - [x] 内`
  画出来贴在左边缘，而同一层的 `  - 内` 缩进一级。根因是缩进量由
  `MarkerOrnament::indent` 携带，而任务项按设计不产标记装饰（`- ` 原样留着，
  这样任务项画成 `- ☐ 待办`、普通项画成 `• 项目`），于是没有任何东西带得动它
  的缩进。前后截图逐字节相同，**是既有缺陷**；第十一刀 b 修好了它旁边那一行，
  它才变得显眼。修法要先回答「谁携带任务项的缩进」——多半是让 `task.rs` 也产
  一条只带 `indent` 的标记装饰，那会让两个 extension 开始互相知道对方存在
  （不变量 D6），值得单独想一想。
