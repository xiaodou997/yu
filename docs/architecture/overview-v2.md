...
这个数把「树归谁」从一次取舍降成一次搬家。搬之前仍然要把
`DecorationCache` 那段模块文档改掉并写明理由——它现在写的是「不放
`MarkdownDocument`」。

#### 还没做的
- **`block_sequence` 与语法树的块结构还没合并。** 第九刀解决了前两个症状
  （task 的 `[ ]` 在树里有节点了、`[x]` 不再是 shortcut `Link`）。**剩下第
  三个**：Setext 标题在块序列里判成段落，于是不放大、不加粗。
  ~~它要块身份两边一致~~——**这句登记是错的**，实测 `标题\n===` 在块序列里
  就是一个块，只是 kind 判错。切法见上面「块结构合并：调查结论」。
