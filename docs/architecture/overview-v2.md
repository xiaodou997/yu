...
- ~~**深色模式不在这一刀里。**~~ **后来的「深色模式」那一刀结掉了**（见下面
  那一节）。留着原文，因为它是一条**登记得对、但理由会过期**的实例：
  ~~调色板是一份写死的浅色，与 `viewport_block_background` 挑同一块底。整个
  编辑区现在就是浅色的：`macos_render_host_config` 把背景写死成
  `Rgba8::white()`……单独给高亮开一条深色路会造出「深色的代码配白色的底」。~~
  理由的前提是「背景永远是白的」；背景跟着 `Appearance` 走之后，前提没了，
  这条登记也就跟着到期。**登记要写清理由，正是为了让它到期时看得出来。**
...
**不算第七刀的一部分**，它是第六刀（导出/导入）的收尾，只是被刀 c 补跑的 G 节
验收捅出来的。刀 c 自己是纯重构，判据是「画面逐字节不变」，这三条行为修复
另起 commit，免得那个判据说不清自己在证明什么。

**三条是一条线上的三层，而且是修下去才知道有三层的：**

| 层 | 症状 | 为什么自动化看不见 |
| --- | --- | --- |
| 剪贴板发出去的 HTML 不声明编码 | 中文粘进 TextEdit 全是乱码 | 所有断言问的都是「HTML 对不对」，而字节一直是对的 |
| 导入白名单把浏览器的**信封**当成「别人的 HTML」 | 网页粘进 Yu 只剩纯文本 | fixture 断言的正是「被拒」，绿着 |
| 平台取剪贴板的顺序是 Markdown > **纯文本** > HTML | HTML 导入路径**一次都没被调用过** | self-check 摆的四种组合里恰好没有「纯文本与可用 HTML 同时在」 |

**第三条是最要紧的那条，而它只有在把前两条修完之后才看得见。** 前两条修好、
单元测试全绿、真实浏览器载荷在 `import_html_fragment` 里能正确翻译成 Markdown
——**而真实窗口里粘出来仍然是纯文本**。那一刻才知道：白名单只是三层里的一层，
下面还压着一条永远走不到的分支。

> **「语料替真实输入说了话」的两个实例，都在这一次。**
> `Fixtures/clipboard/browser-wrapper.html` 断言「被拒」并且绿着；
> clipboard self-check 摆的四种剪贴板组合恰好避开了真实形状。两次都不是判据
> 写错了，是**造出来的输入造不出真实的形状**，于是绿着的断言把一个洞钉成了
> 「有意的」。这与第七刀 spike 那条「只有一类实现的接口等于还没被证明」是
> 同一个形状，只是换到了输入这一侧。

**第一条**：`<meta charset="utf-8">` 加在 `ClipboardPayload::html` 那一层，
不加在 `export_html_fragment` 里——后者的职责是「一段片段」，片段不带编码；
剪贴板是「交给一个不认识的消费者的独立文档」。配套让导入器把顶层的编码声明
当**信封**摘掉，好让第六刀那条「自家导入器接得住自家导出」在剪贴板这条路上
继续成立。

**第二条**：策略从两档改成三档——语义标签照常翻译，**信封与纯呈现**
（`html`/`head`/`body`/`div`/`span`/注释/`<!doctype>`）穿透（`head` 连内容一起
丢，否则页面标题与一整段 CSS 会变成正文），其余标签继续拒。属性**默认忽略**，
理由很干净：**输出是 Markdown，被忽略的属性没有地方可去**。只拒一小份「忽略了
会让输出**静默出错**」的名单：`colspan`/`rowspan`、`reversed`/`type`、`hidden`。
**判据是「忽略它会不会让结果悄悄变错」，不是「我认不认得它」。**

这一条翻了一次登记在案、并且明写着「别当成缺口顺手补上」的决定。翻案的依据是
实测：Chrome 对一个**连一个 `div` 都没有**的纯语义页面，产出的仍然是
`<meta charset='utf-8'><h2 style="…20 条声明…">…<span> </span><strong>…`。
代价（用户写在 Markdown 里的 `<div>raw</div>` 经 HTML 这条路会被拍平成 `raw`）
登记在 `invariants.md` 的 F 节，并由一条**新的**用例钉住，免得下一个人再翻回去。

**第三条**：顺序改成 **Markdown > HTML > 纯文本**。纯文本按定义是同一份内容
丢掉结构之后的样子，两者都在时取纯文本等于每次都主动选那份更少的。回退机制
没有变（`importHTML` 在策略拒绝时返回 `nil`），只是现在真的会走到。

**顺带刨出两个与浏览器无关的解析缺陷，都是「Yu 自己的输出走不到那一支」**：

1. **`hr` 不在空元素名单里。** Yu 的导出器发自闭合的 `<hr />`，`self_closing`
   那条路先接住了它；浏览器发 `<hr style="…">`，于是解析器一直等 `</hr>`，
   等到 EOF 报「格式错误」——**错的是解析器**。
2. **裸布尔属性只放行 `checked` / `disabled`**，其余一律 `Malformed`。
   Yu 只发这两个，而浏览器发的 `hidden` / `draggable` 都是这个形状。

**判据**：最要紧的一条用例吃的是**一次真实浏览器拷贝的原样字节**
（`tests/corpus/chrome-clipboard.html`），不是编出来的 HTML——正因为编不出来，
这个导入器上线以来从没接住过一次。端到端复验：从 Chrome 拷一段带 `<div>` 包裹、
链接、列表、`<hr>` 与表格的网页粘进 Yu，全部成了 Markdown，**表格的居中对齐
也在**。`verify.sh --clean` 十步全绿，14 个 self-check 通过（其中 clipboard
那条的 `browser-wrapper` fixture 从「断言被拒」翻成「断言接住」，并新增一条
...
（D2 不通过：深色外观下侧栏面板变深而**文档区仍是白底**），并且一路卡着
F6、C 节深色、第五刀深色一串验收条目跑不成。

##### 病因不是「忘了配深色」，是「颜色选在了两个地方」

面板会自己变深，因为它们用的是 AppKit 的语义色；文档区不会，因为它是 Rust
画的一帧，而那一帧的颜色来自两处**各自写死的浅色**：

- `macos_render_host_config`（`yu-storage-ffi`）里的背景、正文色与六个装饰色；
- `yu-workspace` 里五个 `viewport_*` 选色函数（代码高亮调色板、表格样式、
  引用条、围栏代码块底、坏图片框）。

后一处的文档一直写着「**产品选色住在这一层**」，而前一处是它唯一的例外
——**例外正是缺陷所在**。

##### 跨 ABI 的是一个事实，不是一套颜色

新增 `yu_workspace::Appearance`（Light / Dark）。平台每帧送**一个字节**，
颜色由 `yu-workspace` 挑。

反过来做（平台送 `textBackgroundColor` 等实际颜色，那正是原注释里写的设想）
会把主题选择挪进壳里，于是**第二端要把同一套配色再挑一遍**，而两端挑出来的
一定会漂开。这条与「唯一实现」是同一条规矩。做完之后
`macos_render_host_config` 里**一个颜色字面量都没有了**——编译器立刻用两个
「未用导入」警告确认了这一点。

##### 它必须进 `FrameKey`，而这是刀 c2 那份定义的第一次实战

换外观**既不推进 Revision 也不改变几何**——正是 `FrameKey` 文档里写的
「新增一种不推进 Revision 的可视状态时必须同时加进来，否则静默跳过」。漏掉它
的表现是「切到深色，面板变深了而正文一动不动」，直到你碰一下文档。

平台那一侧配套一条：`viewDidChangeEffectiveAppearance` 走**既有的**
`onGeometryChange` 重提交路径，而不是另开一条——那条路的语义本来就是「平台
这一侧有什么变了，重新问一次 Rust」。

##### 判据

- **Rust 侧一条用例断三件事**，少任何一件都会放过一个真实缺陷：背景真的换了
  （只改正文色是「白底浅灰字」）、正文色真的换了（只改背景是「深底黑字」）、
  **代码高亮跟着换**（那份调色板以前写死成浅色）。判据取的是**场景层实际发出
  的那一帧**，不是配置里存了什么枚举——存进去而没人读，两者在任何「配置断言」
  下都一样。
- **五个变异全红。** 其中 **Q5 活下来过，是真缺口**：把「认不得的外观字节按
  浅色」改成「按深色」之后全套用例照样绿。补了用例——两种错法差别很大，拒整帧
  是一片空白（违反 I5），猜成深色是白底上的浅字。
- **真实窗口**（Apple M1 Max、macOS 26.5、900×652 逻辑 / 1800×1304 物理，
  文档区行 47–1256、列 0–1769 以避开滚动条那 22 列）：深色系统下**旧版 66%
  是亮像素**（白底），**新版 1%**、97% 是暗像素。代码块的新调色板逐项看过。

##### 一并结掉的登记

第五刀那四条登记里的第一条（「深色模式不在里面……单独给高亮开一条深色路会
造出深色的代码配白色的底」）**理由已经不成立**，因为背景跟着外观走了。深色那
一份**不是把浅色反相**——反相出来的颜色在深底上要么发糊要么刺眼；每一份按自己
那块底挑对比度。

#### `BlockKind` 长出三个变体 —— 已完成

**不属于任何一刀**，是「块结构合并：调查结论」那三刀里第一刀的**收尾**。
第十一刀 b 把「块是什么」整个交给了树，但**变体集合当时刻意没动**（那一刀的
判据是「与今天逐字节相同」）。于是 `---` / 缩进代码 / HTML 块继续映射到
`Paragraph`——三种块认得出来却说不出自己是谁。

`BlockShape::Plain` 的文档一直写着「段落、Setext 标题、ATX 标题、**缩进代码、
分隔线、HTML 块**、引用定义都长这样——它们之间的区别全部由树来分」。位置早就
留好了，这一刀只是把它填上。

| 树的节点 | 新变体 | `viewport_tag` |
| --- | --- | --- |
| `HorizontalRule` | `ThematicBreak` | 8 |
| `CodeBlock`（缩进） | `IndentedCode` | 9 |
| `HtmlBlock` | `HtmlBlock` | 10 |

三件事值得单独说：

1. **`ThematicBreak` 不带拼法。** `---` / `***` / `___` 是同一个变体，与 Setext
   和 ATX 落在同一个 `Heading` 是同一条规矩：**拼法不进块的身份**。
2. **缩进代码与围栏是两个变体，不是一个变体加一个字段。** 围栏带着 `marker`
   与 `closed` 两样负载，缩进代码一样都没有；合成一个会让每个消费者去匹配一个
   对另一半没有意义的字段。
3. **`HtmlBlock` 进 `BlockKind` 不等于 Yu 要渲染 HTML。** 恰恰相反：认出它才能
   **明确地**按源码画（不变量 I5），而不是让它混在段落里被行内语法解析一遍。

##### 两条既有断言变红，都是判据到期

`("    # title\n", BlockKind::Paragraph)` 与
`four_space_indented_definition_remains_literal_paragraph_text` ——四空格缩进的
`#` 与 `[id]: /docs` 在 CommonMark 里**本来就是缩进代码块**，旧断言写
`Paragraph` 是因为当时没有那个变体。两条用例要压的事（「三个空格是标题、四个
不是」「它不是引用定义」）一个都没变，只是现在说得更准。**留着改动痕迹与理由**，
因为「断言变红时先问它钉的是行为还是当时的词汇量」是这一刀唯一的判断。

##### 新用例里最值得留的一条

**缩进代码块是唯一能跨空行的块**（`NodeKind::spans_blank_lines`），所以它是
「叶子节点横跨块边界即片段」那条既有规则第一个真正撞上的形状：`    a\n\n    b\n`
在行扫描器眼里是三块，在树里是一个 `CodeBlock`，三块谁也不完整，全部退回
`Paragraph`。**这不是缺陷，是那条规则在起作用**——认领半个代码块会让画面上出现
两段各画一半的代码。那条规则当初是探针抓出来的，现在有了一条属于它的用例。

##### 判据：分类变了，画面不变

`verify.sh` 十二步全绿，14 个 self-check 全过。

真实窗口截图差分用的是一份**专门含这三种块**的文档（分隔线三种拼法、四空格
缩进代码、`<div>` 块），新旧各拍两张：**文档区 0 像素**。

> **旧版那两张自己差 13,954 px（行 156–245、列 442–798），落在文档区正中。**
> 裁出来一看不是渲染差异：一张里 `#` 露着还有光标竖条，另一张两样都没有——
> **文本视图还没拿到焦点**，于是「光标停在标题里就露出语法」那条行为没发生。
> 新版两张与旧版的**第一张**逐字节一致，跑偏的是旧版的第二张。
> 登记成陷阱 40，并给 `~/yu-shot-tools/` 补了一个 `crop.py`：**差分报出一段
> 行列之后先裁出来看一眼**，光靠数字只会把它读成渲染回归。

##### 这一刀故意不画任何东西

三种块继续按源码原样画。分隔线画成一条线、缩进代码拿到等宽与底色，是下一刀
（下面这一段）的事——判据是截图**故意变了**加人工验收，与这一刀「分类变了、
画面不变」不是同一种判据。混在一个 commit 里就看不出是哪一半动了画面，与第六
刀、刀 c 分两个 commit 同一个理由。

---

#### 把分隔线画成一条线 —— 已完成

**接着上一段。** `BlockKind` 长出三个变体那一刀故意什么都没画（判据是「分类
变了、画面不变」）；这一刀把其中两种画出来：`---` 变成一条横线，四空格缩进的
代码块拿到等宽字体与底色。第三种（`HtmlBlock`）继续按源码原样画——那是有意
的（不变量 I5，而导入那一侧也按白名单拒原始 HTML），不是漏掉。

##### 加了什么

| 层 | 加了什么 |
| --- | --- |
| `yu-markdown` | `BlockOrnament::ThematicBreak`；`extension/thematic_break.rs`；`extension/indented_code.rs` |
| `yu-editor` | `ThematicBreakOrnament`——`BlockOrnaments` 的第四个字段 |
| `yu-workspace` | `viewport_thematic_break_color`；`viewport_block_background` 多认一种 kind |

注册表从十一个 extension 长到十三个。两种语法两个文件，不合成一个：围栏要找
`CodeMark`、算两段隐藏区间、认语言名、按语言名着色，缩进代码一样都没有——
合成一个文件的话每一段都要先问「我现在是哪一种」，而这正是 `BlockKind` 把它们
分成两个变体的理由。
...
标题不是这样：`#` 藏起来了，字号照样放大，两件事互不妨碍。分隔线不同——那条
线占的**就是**正文那一行。所以两样一起由 `is_focus` 决定：**光标进来是源码，
光标出去是一条线**。

只做一半都是缺陷，而且两种错法都不报错：只露源码不撤线，线正好穿过那三个减号
，画面上是一条删除线；只撤线不露源码，用户按退格会删掉一个他看不见的字符（与
`heading.rs` 露出 `##` 是同一条理由）。**做法是让它们出自同一个事实**——
`yu-markdown` 在焦点块上一条装饰都不产，于是 `yu-editor` 没有可翻译的东西、
场景层没有可画的东西，下游两层一次焦点都不用判。

**二、横线用既有的 `OrnamentRole::Border`，不新开一个角色。**

场景层的词汇是**渲染中立**的（§2.1）：`Border` 就是一条边框线，不管它属于表格、
任务框还是分隔线，后端对它们做的事一模一样——落成一个实心矩形。为分隔线新开
一个角色就是「一种语法一条全链路」那种泄漏的第一步。

**三、缩进代码行首那四个空格留在画面上。**

树给的 `CodeBlock` 节点**只跳过第一行的缩进**（`    only\n` 里块是 `0..9`、
节点是 `4..8`），后面每一行的四个空格都在节点里面。要藏就得自己逐行扫一遍
缩进——把行扫描器的活儿在这里重写一遍，而「块的边界由谁定」是已经登记在闸门
后面的那件事。按源码原样画是 I5 的缺省，代价只是正文比围栏里的正文往右四格。

##### 一件明确不做的：缩进代码不着色

它没有语言名可问。`fenced_code.rs` 里没有 info 的围栏也是一个字都不着色，这是
同一条规矩，不是遗漏——写进 `indented_code.rs` 的模块文档，免得下次有人当成
缺口顺手补。

##### 定义域这一条挡住了什么

`- a\n***\n` 在行扫描器眼里是**一个**块，在树里是 `BulletList` 加一个
`HorizontalRule`，谁也装不下它，于是 `classify` 退回 `Paragraph`。而
`BlockContext::block_node` 在块自己的节点对不上时会**往块内找**——不问
`BlockKind` 直接问节点的话，这一块会被藏掉正中间那三个星号，再横着画一条线
穿过整块。

**「Setext 的 `---` 不是分隔线」那条用例挡不住这种写法**（Setext 块里根本没有
`HorizontalRule` 节点），所以两条用例都要有：一条挡「自己扫文本找 `^---$`」，
一条挡「问节点不问 kind」。这是「两道门挡同一件事时，里面那道往往没有自己的
用例」的反面——两道门挡的其实是两件事。

##### 真实窗口抓出来的一处：三条线粗细不一样

**这条自动化用例一条都压不住，只有真实窗口看得见。** 语料里三条只有拼法不同
的分隔线（`---` / `***` / `___`），截出来第一条比另外两条**粗 50%**——2 个
物理像素 vs 3 个。

病因是块的纵坐标是一路累加出来的浮点数，同一条名义上一个逻辑像素粗的线，落在
`y = k` 上占 2 个物理像素、落在 `y = k + 0.25` 上占 3 个（900×652 逻辑 /
1800×1304 物理，2× Retina）。后端画的是「这个矩形覆盖到的物理像素」，粗细本身
没有错，错的是它出现在了一个非整数的纵坐标上。

修法是把横线的矩形在 `yu-workspace` 里**对齐到整逻辑像素**（只动纵坐标，粗细
已经是常数、横向本来就铺满正文栏）。对齐只做在这一条线上，不做在引用竖条或
表格网格线上——它们有同一个毛病，但这一刀的截图判据是「除了分隔线与缩进代码
那几行，别的一个像素都不许动」，顺手对齐它们会让判据说不清是哪一半动了画面，
登记在案。

**这是「拼法不进语义」这句话在画面上的样子**：三种拼法产出同一条
`BlockOrnament::ThematicBreak`，如果画出来粗细不一样，说明拼法还是从什么地方
漏了进来（这次漏的其实是纵坐标的浮点零头，不是拼法本身，但表现完全一样）——
提醒的是「语义层的判断不能只靠单元测试验证，画出来是不是真的一样，只有真实
窗口才答得上来」。

##### 判据

`verify.sh` 十二步全绿。`yu-markdown` / `yu-editor` / `yu-workspace` 三个 crate
新增 14 条用例（8 + 3 + 3），10 组反向验证全红（焦点态两个方向、定义域两条、
缩进代码两条、几何三条、场景层两条）。

真实窗口截图（Apple M1 Max、macOS 26.5、900×652 逻辑 / 1800×1304 物理，语料含
三种拼法的分隔线、一段四空格缩进代码、一段围栏代码、首尾各一段正文）：同一个
二进制两拍噪声底 0；新旧对比，差异**只落在**三条分隔线与缩进代码块那几行
（分隔线：字面减号消失、线出现在块中央；缩进代码：着上等宽字体与底色），别的
每一行都是 0——包括分隔线与代码块**下面**的正文，块高没有被这一刀改变。深浅
两种外观各拍一遍，线与底色都对得上主题色。
