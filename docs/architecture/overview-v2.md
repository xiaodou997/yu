...
**进行中。** 九刀落地：`yu-markdown` 成为 extension 集合；`BlockLayoutInput`
...
TaskList 进 `yu-syntax`。剩下的是把 `block_sequence` 与语法树的块结构合并。
...
#### 还没做的
- **`block_sequence` 与语法树的块结构还没合并。** 第九刀解决了前两个症状
  （task 的 `[ ]` 在树里有节点了、`[x]` 不再是 shortcut `Link`）。**剩下第
  三个**：Setext 标题两边都不认（`标题\n===` 在块序列里是两个块，在树里是一个
  `SetextHeading1`），它要块身份两边一致。
- **同一个「这是不是任务项」还有两个实现。** `BlockKind::TaskListItem` 与
  树的 `Task`，由 `yu-markdown/tests/task_identity.rs` 锁着。已登记的不一致
  只有一处：`block_sequence` 不下降到容器里，引用块里的任务项没有复选框
  （v1 起就是这样）。合并块结构时一起收掉。
- **新发现（截图抓到的，与这一刀无关）：复选框盖住了正文的第一个字。**
  ...
  图一个像素都没有」是同一类。修它的正路是让复选框成为
  `Decoration::Widget`（第 3 节对照表本来就是这么写的），那要先解决
  「非空 range 的 widget 会隐藏它覆盖的 source」——与表格那一格同一个问题。
- **新发现（与任务项无关，属于上面那条 Setext 线）：Setext 下划线那一行的
  ...
  所以一直没被抓到。第九刀的探针撞上的，**没有在这一刀里修**：它落在 Setext
  那条线上，与块身份合并是同一件事，混进来会让归因失效。
