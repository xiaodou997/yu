...
> 第六轮更正：本页三套多文档压力所用的旧进程字符串匹配漏掉 `Contents/MacOS/../Helpers/` 启动路径。其 helper 数量、helper 物理足迹、空闲退出和无残留断言不能继续算作已验证；源码、历史、像素和应用足迹证据保留。原始 JSON 不篡改，修复及重新验证见 [第六轮](mac-group4-followup-06.md)。不由此否定采用不同观察器的冷重开／主题专项。
