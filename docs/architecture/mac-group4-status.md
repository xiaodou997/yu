...
本批工作方式：优先交付到 GitHub `main`。第七批在 `yu-workspace` 的干净基线 `f891e971bf365fbb75d422f0a0033eb404a79c2c` 上实现 Gantt 日期时间/刻度和本地日历刷新，并执行 helper、FFI、独立 Swift 日历检查及宿主类型检查。GitHub CI 因额度不可用，本批不修改 CI 配置。核心检查与应用打包、真实跨午夜/睡眠唤醒及实窗验收分开记录；本次没有冻结或打包应用。2026-09-21 的桌面权限预检仍只作为历史记录。
...
| 第六批：时序中心连接与生命周期 | 已合入 `f891e971`：三种 `()` 位置、保留箭头语义、圆几何、激活时序和非法状态校验 | 当批 helper 94项通过；14项时序集成及真实 helper 恢复回归继续纳入第七批重跑；[记录](mac-group4-sequence-central.md)，实窗未执行 |
| 第七批：Gantt 日期时间与日历刷新 | 本页同批提交：数字日期/时分秒/毫秒、12小时制、子日/月刻度、帧复用前日历检查、单次日边界唤醒及睡眠/时区处理 | helper 111项、日历FFI 2项、Swift 7组通过；Clippy与宿主Swift类型检查通过；[边界与验收](mac-group4-gantt-calendar.md)，应用打包/真实跨日与实窗未执行 |
...
4. **图表组合与日历实窗验收**：时序中心连接和第七批 Gantt 日期时间/刻度/日历刷新已进入代码和核心回归，不再重复列作基本链路未实现。仍需最新构建验证真实跨日、睡眠/时区/隐藏恢复、迟到资源丢弃和继续编辑；不支持的时区偏移、Unix时间戳、月份名输入等边界见[第七批记录](mac-group4-gantt-calendar.md)。
...
真机使用 [group4-gantt-calendar.md](../../platform/macos/yu-shell-macos/Fixtures/group4-gantt-calendar.md) 的隔离副本：前四段有效，后两段为预期诊断。独立 Swift 日历测试已纳入自检脚本，但本轮不声称整份应用自检已执行。纯时间图使用宿主本地日期，显式日期不随今日重排；民用时间轴不是带时区的绝对时刻模型。精确结果、支持边界与操作步骤见[第七批记录](mac-group4-gantt-calendar.md)。下一阶段冻结构建后执行统一实窗与资源审计，不提前进入第五组导出。
