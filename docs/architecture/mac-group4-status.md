...
本批工作方式：优先交付到 GitHub `main`。第五批在 `yu-workspace` 的干净基线 `db222c955d8f8c97c2538247f9b11dae7b00d199` 上修改并执行核心回归。GitHub CI 因额度不可用，本批不修改 CI 配置。实际运行的 Rust/Python 检查与应用打包、实窗验收分开记录；本次未打包应用或执行实窗验收。2026-09-21 的桌面权限预检仍只作为历史记录。
...
| 第五批：HTML 跨行组粘贴 | 本页同批提交：逐 owner 前置边界校验、结果形状校验、保组全选覆盖和选区约束 | 新增 14 项跨行组集成回归；相关四个 crate 853 项通过，详细静态检查及边界见[本批记录](mac-group4-table-row-groups.md)；打包/实窗未执行 |
