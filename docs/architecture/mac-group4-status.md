...
本批工作方式：优先交付到 GitHub `main`。本次 `yu-workspace` 插件已可访问，工作区由干净的历史版本快进对齐 `452e89f7`，用于源码修改及核心回归。GitHub CI 因额度不可用，本批不修改 CI 配置。实际运行的 Rust/Python 检查与应用打包、实窗验收分开记录；本次未打包应用或执行实窗验收。2026-09-21 的桌面权限预检仍只作为历史记录。

## 最近四批：代码状态与验证状态分开

| 批次 | 代码状态 | 本地检查与实窗状态 |
| --- | --- | --- |
| 第一批：粘贴拒绝原子性 | 已包含在基线 `0100eb8d`；[记录与标准](mac-group4-paste-atomicity.md) | 不将历史记录当作本次重跑；普通公式、合法脚注拒绝预期分别由第三/四批替换；非法输入和跨行组拒绝保留 |
...
| 第三批：公式表格转换 | 已合入 `452e89f7`；逐单元格保留 TeX、原生投影与编辑 | 本轮修正集成测试调用私有方法的问题，10 项公式回归已通过；实窗未验收；[记录](mac-group4-table-math.md) |
| 第四批：脚注表格转换 | 本页同批提交：原标签保留、逐格校验、全文编号/重复引用/定义回链、原生投影及成功/拒绝回归 | 新增 16 项脚注集成测试；核心检查结果见[本批记录](mac-group4-table-footnotes.md)，打包与实窗未执行 |

第四批接通了有唯一文档外定义的脚注转换；缺失/重复定义、解析器分歧及非法 HTML 仍安全拒绝。跨行组和全部公式语法没有顺带放开。核心测试通过不等于实窗验收通过；测试真机仍需记录实际提交 SHA、命令结果及应用/helper 哈希。
...
| 结构编辑 | 合并表格布局/导航/结构修改/剪贴板往返；Markdown 表格接收合并内容；HTML 跨层级列表选区编辑；行内公式及脚注表格转换的源码保留、原生接入与定义关联 |
...
1. **公式/脚注转换实窗验证**：核心实现及回归已交付，在测试真机继续打包及浅深色交互，核对真实剪贴板、中文/emoji、混排、未选中引用、BOM/CRLF、一次撤销重做、保存完全退出重开及拒绝原子性。
...
输出目录必须尚不存在。准备脚本现生成 52 份隔离样本，manifest 版本 3：合法公式/脚注及混排为成功预期，非法公式、缺失/重复脚注定义、非法脚注标记及跨行组为拒绝预期。全部实窗状态初始为 `not_run`；生成文件不等于执行测试。核心检查、操作细节和边界见[第四批验收标准](mac-group4-table-footnotes.md)。
