...
最新收尾第一轮：`e4e2cf35` 修复列宽分隔线AX坐标漏加阅读区边距；`c747ea13` 修复跨条目拖选中HTML标签展开导致命中漂移。两项均独立提交并推送main。最终受测应用SHA256为 `3e5548cf175569231eebd61973670ee213b80f08590c34d329a1d5c7ac8e6ace`，与旧RC3不同；本轮相关三个crate 872项及19项原生自检通过，最终同产品构建7组／55条脚本检查通过，实际补整表正反选择／空组／列宽、父子反向拖选／多光标、综合文档TOC／details与300秒单文档73轮资源循环。内存样本仍增长，不作长期无泄漏结论。精确结果及有限压力边界见[收尾第一轮](mac-group4-followup-01.md)。旧冻结标签不移动。

上一轮冻结构建：`group4-freeze-20260926-rc3`，源码 `6afb7a3dac785d2674236cff1fab65f3546fee39`，应用 SHA256 `ba4caa17724a5813a5c81d208311bd30a42683035773c8fec8187a3b52df539d`。本轮在 `yu-workspace` 实际完成 Release 打包／产物审计、1541项 Rust 回归、19项原生自检及16组浅深色／资源实窗矩阵；图像复核57张可见窗口。RC2 发现公式表格转换只保留源码却未进入生产公式索引，已修复并以 RC3 重跑。真实跨午夜／多日睡眠、长时压力及余下完整组合仍未验收，第四组不结项。精确哈希、失败与重跑证据见[冻结审计](mac-group4-freeze-audit.md)，最近全部材料见[测试文档索引](mac-group4-test-index.md)。GitHub CI 不可用且未修改配置；桌面预检本轮实际通过，不再以旧锁屏记录描述当前状态。

## 最近七批功能、第八批冻结审计与后续收尾：分开记录
...
1. **余下表格／列表窗口排列**：收尾第一轮已补整表正／反向真实拖选、跨组成功／拒绝、空tbody保留、55pt合并列宽拖动／取消／重开，以及独立／单元格的父子反向拖选和Option双光标。更多编码、主选区／多范围、缩放及76／96份全排列仍按缺口跟踪，不再把已跑组合重复列作全部未执行。
2. **脚注转换支持边界**：已支持唯一文档外定义、重复引用和跳转；字面实体标签的转换器分歧仍明确拒绝，不自动导入/重命名外来定义。下一批不再重复开发已接通的基本链路。
3. **整份综合文档与相邻流程**：收尾第一轮已在完整 `group4-smoke.md` 实测目录点击、details开关、隐藏正文搜索／编辑／历史和完全重开，浅深色通过。只点击了一个目录链接，按标题取景不保证所有屏外长页全覆盖；所有既有写作流程和长时序图全图仍需补验。`blockquote`仍是有限HTML的源码回退边界。
...
5. **长期资源与最终签收**：RC3的生命周期／取消证据保留；本轮增加固定300秒单文档有效／错误／普通内容循环，结果按实际二进制单列。有限窗口内的内存与进程观测不替代长期、多文档压力或泄漏证明。真实系统日期事件未完成，第四组保持未结项。
