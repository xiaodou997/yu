...
- 目标限定为当前 Markdown block 的 `LayoutSnapshot::lines()`。目标行通过
  `LayoutSnapshot::hit_test(LayoutPoint { x: preferred_x, y: target_line.y })` 反向得到 source
  boundary，再创建新的 `EditorSelection`；命令不创建 Transaction、Revision 或 history entry。
...
- 非空 selection 的 Up/Down 先分别折叠到 ordered start/end。横向/word 移动、永久 edit、
  显式 selection、composition 边界和 reset 都清除 preferred-X；跨 block/viewport 导航留给
  后续阶段。
...
- 长行 → 短行 → 长行的连续下移会回到原始 X；行首/行尾和空 block 不会伪造跨 block 导航。
...
同一 block 提供一致的 shaping/layout backend，并另外定义跨 block、viewport 自动滚动、Shift
扩展 selection、Page Up/Down 和 RTL 视觉行策略。
