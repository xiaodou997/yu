...
当活动 `CompositionOverlay` 的 replacement range 完全落在一个 Markdown block 内时：

1. `EditorDocument::composition_block_index` 负责确认 block-local 边界；跨 block replacement
   不构造部分 scene，继续使用 native source mirror 回退。
2. `CoreTextViewportFrameBuilder` 对该 block 使用不进入 `LayoutCache` 的
...
4. diagnostic glyph/render-plan bridge 使用相同 block 选择规则，避免 publication 与 count/fill
...
`NSTextInputClient`、Accessibility 和失败回退宿主，跨 block layout、完整 visual renderer
替换和最终输入接管留到后续阶段。
...
- cancel 会再次发布 canonical glyph scene；跨 block composition 不会发布不完整的 scene。
