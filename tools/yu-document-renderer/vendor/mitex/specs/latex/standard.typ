#import "../../../xarrow/lib.typ": xarrow

#import "../prelude.typ": *

// 0. Some useful internal variables or functions
// TeX cases separates the expression and condition columns by one quad.
// Typst alignment points have no intrinsic horizontal separation.
#let tex-cases(reverse: false, ..rows) = {
  let alignment-point = $&$.body.func()
  let separate(row) = {
    if row.has("children") {
      row.children.map(it => if it.func() == alignment-point { it + h(1em) } else { it }).sum(default: [])
    } else { row }
  }
  math.cases(reverse: reverse, ..rows.pos().map(separate))
}
#let mitex-color-map = (
  "red": rgb(255, 0, 0),
  "green": rgb(0, 255, 0),
...
#let (
  ...
  cases: define-env(none, kind: "is-cases", alias: "cases", handle: tex-cases),
  rcases: define-env(none, kind: "is-cases", alias: "rcases", handle: tex-cases.with(reverse: true)),
  // Specials
  notag: ignore-sym,
  relax: ignore-sym,
  cr: ignore-sym,
  ...
  nonumber: ignore-sym,
  KaTeX: of-sym(math.upright("KaTeX")),
