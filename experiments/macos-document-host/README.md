...
Metal surface 已能消费受影响 block span 的 transient shaped preedit glyph；跨 block replacement
...
还会消费 source-backed cell layout、column/row divider hit-test 和 session-only column
geometry override，并验证 canonical layout/source 不变、tolerance 和 stale Revision 拒绝。
当前只建立 block-local projection 诊断边界，生产 TextKit mirror 和完整 visual delimiter
语义保持不变。
...
composition 会在同一持久 publication 中重建受影响 block span 的 shaped preedit glyph。
...
淘汰。snapshot 还会区分 image candidate、去重后的 request、visible/overscan 分布和退避 retry
次数，便于确认重复 destination 没有重复排队。composition generation 变化会强制提交新的
