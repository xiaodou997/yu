...
Revision 自动请求 caret reveal。TextKit container width、CoreText system UI line height 和混合
grapheme sample 的 shaped advance 会通过 revision-bound FFI metrics 配置发送到 Rust，caret
request 直接使用 native point，不再依赖 Swift scale。system UI 私有 `.SFNS-*` 名称不会穿过
普通 family lookup；FFI 只返回 owned scalar。该 advance 仍只服务 metrics-only backend，正式
布局仍需接入共享 shaped-layout metrics。
