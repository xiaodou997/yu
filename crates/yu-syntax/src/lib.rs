...
//! # 增量的代价在哪里
//!
//! 一次单字符编辑**重新扫描的字节数与文档大小无关**（1 MiB 与 4 MiB 的文档
//! 实测都是 65~68 字节），但**耗时仍然随文档大小增长**（约 0.6 ms 与
//! 1.3 ms）。两者不矛盾：复用不需要重新扫描字节，但需要把每一个被复用的
//! 顶层块逐个重新挂进新的 Document 节点——`FragmentCursor::take_nodes` 是
//! 按兄弟节点一个一个走的，上游也是。
//!
//! 也就是说增量解析目前是 **O(块数)** 而不是 O(改动量)。相对全量解析仍然
//! 快一个数量级以上（1 MiB：12.7 ms → 0.6 ms），对百万行以内的文档够用。
//! 真要去掉这一项，Document 的子节点得换成可持久化的序列结构而不是 `Vec`，
//! 那是一件独立的事，不在 S3 范围内。
//!
//! 记在这里是因为它会影响 S5：如果布局也按「整棵树重新遍历」组织，两个
//! O(块数) 会叠加。
