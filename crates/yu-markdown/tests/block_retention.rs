...
fn explicit_compaction_packs_a_segmented_document() {
    for backend in StorageBackend::ALL {
        ...
        assert_eq!(document, semantic_copy, "backend {backend}");
    ...
    for backend in StorageBackend::ALL {
        ...
        for step in 0..500 {
            ...
            assert!(
                ...
                "backend {backend} exceeded policy at step {step}"
            );

            if step.is_multiple_of(25) {
                assert_eq!(
                    document,
                    parse(applied.result_snapshot()),
                    "backend {backend}"
                );
            }
        }

        assert!(compactions > 0, "backend {backend} never compacted");
        assert_eq!(buffer.snapshot().as_str(), model, "backend {backend}");
        assert_eq!(document, parse(&buffer.snapshot()), "backend {backend}");
